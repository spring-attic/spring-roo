...
public class MongoCommands implements CommandMarker {
	...
	@Reference private MongoOperations repositoryMongoOperations;
	
	@CliAvailabilityIndicator("mongo setup")
	public boolean isMongoSetupCommandAvailable() {
		return repositoryMongoOperations.isSetupCommandAvailable();
	}
	
	@CliAvailabilityIndicator({"repository mongo", "entity mongo"})
	public boolean isRepositoryCommandAvailable() {
		return repositoryMongoOperations.isRepositoryCommandAvailable();
	}
	
	@CliCommand(value = "mongo setup", help = "Configures the project for MongoDB peristence.") 
	public void setup(@CliOption(key = "username", mandatory = false, help = "Username for accessing the database (defaults to '')") String username,
		...
		repositoryMongoOperations.setup(username, password, name, port, host, cloudFoundry);
	}
