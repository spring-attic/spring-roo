...
This format is applied automatically to the entire project using http://mvnrepository.com/artifact/com.googlecode.maven-java-formatter-plugin/maven-java-formatter-plugin[Maven Java Formatter plugin] and the https://github.com/google/styleguide/blob/gh-pages/eclipse-java-google-style.xml[Eclipse Java Code Format definition .xml file] included on _devel-resources_ folder.
 
Spring Roo formats all _.java_ files when developer executes `mvn clean install`. 

IMPORTANT: _Contributors must execute `mvn clean install` before sending some new pull request to be sure that Google Code Styles have been applied._
