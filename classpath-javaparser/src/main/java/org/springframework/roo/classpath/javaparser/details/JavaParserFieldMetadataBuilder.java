...
public class JavaParserFieldMetadataBuilder implements Builder<FieldMetadata> {
    ...
    private JavaParserFieldMetadataBuilder(final String declaredByMetadataId,
        ...
        this.modifier = JavaParserUtils.getJdkModifier(fieldDeclaration
                .getModifiers());

        this.declaredByMetadataId = declaredByMetadataId;

        Type type = fieldDeclaration.getType();
        this.fieldType = JavaParserUtils.getJavaType(compilationUnitServices,
                type, typeParameters);

        // Convert into an array if this variable ID uses array notation
        if (var.getId().getArrayCount() > 0) {
            this.fieldType = new JavaType(
                    fieldType.getFullyQualifiedTypeName(), var.getId()
                            .getArrayCount() + fieldType.getArray(),
    ...
    public static void addField(
        ...
        ClassOrInterfaceType initType = JavaParserUtils.getResolvedName(
                compilationUnitServices.getEnclosingTypeName(),
                field.getFieldType(), compilationUnitServices);

        FieldDeclaration newField = ASTHelper.createFieldDeclaration(
        ...
        if (field.getFieldType().getParameters().size() > 0) {
            ...
            for (JavaType parameter : field.getFieldType().getParameters()) {
                NameExpr importedParameterType = JavaParserUtils
        ...
        if (StringUtils.hasText(field.getFieldInitializer())) {
            ...
            if (init instanceof ObjectCreationExpr) {
                ...
                JavaType typeToImport = JavaParserUtils.getJavaTypeNow(
                        compilationUnitServices, ocr.getType(), null);
                NameExpr nameExpr = JavaParserUtils.importTypeIfRequired(
                        compilationUnitServices.getEnclosingTypeName(),
                        compilationUnitServices.getImports(), typeToImport);
                ClassOrInterfaceType classOrInterfaceType = JavaParserUtils
