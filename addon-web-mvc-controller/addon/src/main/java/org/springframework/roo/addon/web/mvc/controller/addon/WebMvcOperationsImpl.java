...
public class WebMvcOperationsImpl implements WebMvcOperations {
  ...
  private static final String CONVERSION_SERVICE_EXPOSING_INTERCEPTOR_NAME =
      "conversionServiceExposingInterceptor";
  private static final String CONVERSION_SERVICE_SIMPLE_TYPE =
  ...
  private void copyWebXml() {
    ...
    Validate.isTrue(fileManager.exists(pathResolver.getFocusedIdentifier(Path.SRC_MAIN_WEBAPP,
        servletCtxFilename)), "'%s' does not exist", servletCtxFilename);
    ...
    final InputStream templateInputStream =
        FileUtils.getInputStream(getClass(), "web-template.xml");
  ...
  private void createWebApplicationContext() {
    ...
    fileManager.createOrUpdateTextFileIfRequired(webConfigFile, XmlUtils.nodeToString(document),
        true);
  }
  ...
  public void installConversionService(final JavaPackage destinationPackage) {
    final String webMvcConfigPath =
        pathResolver.getFocusedIdentifier(Path.SRC_MAIN_WEBAPP, WEBMVC_CONFIG_XML);
    Validate.isTrue(fileManager.exists(webMvcConfigPath), "'%s' does not exist", webMvcConfigPath);

    final Document document = XmlUtils.readXml(fileManager.getInputStream(webMvcConfigPath));
    final Element root = document.getDocumentElement();

    final Element annotationDriven = DomUtils.findFirstElementByName("mvc:annotation-driven", root);
    ...
    final Element conversionServiceBean =
        new XmlElementBuilder("bean", document)
    ...
    fileManager.createOrUpdateTextFileIfRequired(webMvcConfigPath, XmlUtils.nodeToString(document),
        false);
  ...
  private void installConversionServiceJavaClass(final JavaPackage thePackage) {
    final JavaType javaType =
        new JavaType(thePackage.getFullyQualifiedPackageName()
            + ".ApplicationConversionServiceFactoryBean");
  ...
  private boolean isConversionServiceConfigured() {
    final String webMvcConfigPath =
        pathResolver.getFocusedIdentifier(Path.SRC_MAIN_WEBAPP, WEBMVC_CONFIG_XML);
    Validate.isTrue(fileManager.exists(webMvcConfigPath), "'%s'  doesn't exist", webMvcConfigPath);

    final MutableFile mutableFile = fileManager.updateFile(webMvcConfigPath);
    final Document document = XmlUtils.readXml(mutableFile.getInputStream());
    final Element root = document.getDocumentElement();

    final Element annotationDrivenElement =
        DomUtils.findFirstElementByName("mvc:annotation-driven", root);
  ...
  private boolean isConversionServiceConfigured(final Element root,
      final Element annotationDrivenElement) {
    final String beanName = annotationDrivenElement.getAttribute("conversion-service");
    if (StringUtils.isBlank(beanName)) {
      return false;
    }

    final Element bean = XmlUtils.findFirstElement("/beans/bean[@id=\"" + beanName + "\"]", root);
    final String classAttribute = bean.getAttribute("class");
    final StringBuilder sb =
        new StringBuilder("Found custom ConversionService installed in webmvc-config.xml. ");
    ...
    Validate.isTrue(classAttribute.endsWith(CONVERSION_SERVICE_SIMPLE_TYPE), sb.toString());
    return true;
  }
  ...
  private void manageWebXml() {
    ...
    WebXmlUtils.addContextParam(new WebXmlUtils.WebXmlParam("defaultHtmlEscape", "true"), document,
    ...
    WebXmlUtils.addFilter(CHARACTER_ENCODING_FILTER_NAME, CHARACTER_ENCODING_FILTER
        .getFullyQualifiedTypeName(), "/*", document, null, new WebXmlUtils.WebXmlParam("encoding",
        "UTF-8"), new WebXmlUtils.WebXmlParam("forceEncoding", "true"));
    WebXmlUtils.addFilter(HTTP_METHOD_FILTER_NAME,
        HIDDEN_HTTP_METHOD_FILTER.getFullyQualifiedTypeName(), "/*", document, null);
    if (projectOperations.isFeatureInstalled(FeatureNames.JPA)) {
      WebXmlUtils.addFilter(OPEN_ENTITYMANAGER_IN_VIEW_FILTER_NAME,
          OPEN_ENTITY_MANAGER_IN_VIEW_FILTER.getFullyQualifiedTypeName(), "/*", document, null);
    }
    WebXmlUtils.addListener(CONTEXT_LOADER_LISTENER.getFullyQualifiedTypeName(), document,
        "Creates the Spring Container shared by all Servlets and Filters");
    WebXmlUtils.addServlet(projectOperations.getFocusedProjectName(), DISPATCHER_SERVLET
        .getFullyQualifiedTypeName(), "/", 1, document, "Handles Spring requests",
        new WebXmlUtils.WebXmlParam("contextConfigLocation", WEBMVC_CONFIG_XML));
    WebXmlUtils.setSessionTimeout(10, document, null);
    WebXmlUtils.addExceptionType(EXCEPTION.getFullyQualifiedTypeName(), "/uncaughtException",
        document, null);
    WebXmlUtils.addErrorCode(new Integer(404), "/resourceNotFound", document, null);

    fileManager
        .createOrUpdateTextFileIfRequired(webXmlPath, XmlUtils.nodeToString(document), false);
  }
  ...
  public void registerWebFlowConversionServiceExposingInterceptor() {
    final String webFlowConfigPath =
        pathResolver
            .getFocusedIdentifier(Path.SRC_MAIN_WEBAPP, "WEB-INF/spring/webflow-config.xml");
    ...
    final Document document = XmlUtils.readXml(fileManager.getInputStream(webFlowConfigPath));
    final Element root = document.getDocumentElement();

    if (XmlUtils.findFirstElement("/beans/bean[@id='"
        + CONVERSION_SERVICE_EXPOSING_INTERCEPTOR_NAME + "']", root) == null) {
      final Element conversionServiceExposingInterceptor =
          new XmlElementBuilder("bean", document)
              .addAttribute("class",
                  CONVERSION_SERVICE_EXPOSING_INTERCEPTOR.getFullyQualifiedTypeName())
              .addAttribute("id", CONVERSION_SERVICE_EXPOSING_INTERCEPTOR_NAME)
              .addChild(
                  new XmlElementBuilder("constructor-arg", document).addAttribute("ref",
                      CONVERSION_SERVICE_BEAN_NAME).build()).build();
      root.appendChild(conversionServiceExposingInterceptor);
    }
    final Element flowHandlerMapping =
        XmlUtils.findFirstElement(
            "/beans/bean[@class='" + FLOW_HANDLER_MAPPING.getFullyQualifiedTypeName() + "']", root);
    if (flowHandlerMapping != null) {
      if (XmlUtils.findFirstElement("property[@name='interceptors']/array/ref[@bean='"
          + CONVERSION_SERVICE_EXPOSING_INTERCEPTOR_NAME + "']", flowHandlerMapping) == null) {
        final Element interceptors =
            new XmlElementBuilder("property", document)
                .addAttribute("name", "interceptors")
                .addChild(
                    new XmlElementBuilder("array", document).addChild(
                        new XmlElementBuilder("ref", document).addAttribute("bean",
                            CONVERSION_SERVICE_EXPOSING_INTERCEPTOR_NAME).build()).build()).build();
  ...
  private void setBasePackageForComponentScan(final Document document) {
    final Element componentScanElement =
        DomUtils.findFirstElementByName("context:component-scan",
  ...
  private void updateConfiguration() {
    ...
    final String webConfigFile =
        pathResolver.getFocusedIdentifier(Path.SRC_MAIN_WEBAPP, WEBMVC_CONFIG_XML);
    Validate
        .isTrue(fileManager.exists(webConfigFile), "Aborting: Unable to find %s", webConfigFile);
    ...
    for (final Element dependencyElement : springDependencies) {
      ...
      if (isGaeEnabled && dependency.getGroupId().equals("org.glassfish.web")
          && dependency.getArtifactId().equals("jstl-impl")) {
        dependencies.add(new Dependency(dependency.getGroupId(), dependency.getArtifactId(),
            dependency.getVersion(), DependencyType.JAR, DependencyScope.PROVIDED));
      } else {
