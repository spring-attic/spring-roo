...
public class WebJsonOperationsImpl implements WebJsonOperations {
  ...
  public void annotateAll(JavaPackage javaPackage) {
    ...
    for (final ClassOrInterfaceTypeDetails cod : typeLocationService
      ...
      for (final ClassOrInterfaceTypeDetails mvcCod : typeLocationService
        ...
        // just found it.
        final PhysicalTypeMetadata mvcMd =
            (PhysicalTypeMetadata) metadataService.get(mvcCod.getDeclaredByMetadataId());
        final WebScaffoldAnnotationValues webScaffoldAnnotationValues =
            new WebScaffoldAnnotationValues(mvcMd);
        if (webScaffoldAnnotationValues.isAnnotationFound()
            && webScaffoldAnnotationValues.getFormBackingObject().equals(jsonType)) {
      ...
      if (mvcType == null) {
        createNewType(
            new JavaType(javaPackage.getFullyQualifiedPackageName() + "."
                + jsonType.getSimpleTypeName() + "Controller"), jsonType);
      } else {
  ...
  private void appendToExistingType(final JavaType type, final JavaType jsonEntity) {
    final ClassOrInterfaceTypeDetails cid = typeLocationService.getTypeDetails(type);
    if (cid == null) {
  ...
  private void createNewType(final JavaType type, final JavaType jsonEntity) {
    final PluralMetadata pluralMetadata =
        (PluralMetadata) metadataService.get(PluralMetadata.createIdentifier(jsonEntity,
    ...
    final String declaredByMetadataId =
        PhysicalTypeIdentifier.createIdentifier(type,
            pathResolver.getFocusedPath(Path.SRC_MAIN_JAVA));
    final ClassOrInterfaceTypeDetailsBuilder cidBuilder =
        new ClassOrInterfaceTypeDetailsBuilder(declaredByMetadataId, Modifier.PUBLIC, type,
    ...
    requestMapping.addAttribute(new StringAttributeValue(new JavaSymbolName("value"), "/"
  ...
  private AnnotationMetadataBuilder getAnnotation(final JavaType type) {
    ...
    final List<AnnotationAttributeValue<?>> rooJsonAttributes =
  ...
  public void setup() {
    ...
    final String webXmlPath =
        pathResolver.getFocusedIdentifier(Path.SRC_MAIN_WEBAPP, "WEB-INF/web.xml");
    ...
    WebXmlUtils.addFilter(WebMvcOperations.CHARACTER_ENCODING_FILTER_NAME,
        CHARACTER_ENCODING_FILTER.getFullyQualifiedTypeName(), "/*", document, null,
        new WebXmlUtils.WebXmlParam("encoding", "UTF-8"), new WebXmlUtils.WebXmlParam(
            "forceEncoding", "true"));
    ...
    WebXmlUtils.addServlet(projectOperations.getFocusedProjectName(), DISPATCHER_SERVLET
        ...
        new WebXmlUtils.WebXmlParam("contextConfigLocation", "WEB-INF/spring/webmvc-config.xml"));
  ...
  private void updateConfiguration() {
    ...
    final List<Element> springDependencies =
        XmlUtils
            .findElements("/configuration/springWebJson/dependencies/dependency", configuration);
