...
public class WebFinderMetadata extends AbstractItdTypeDetailsProvidingMetadataItem {
   ...
   */
  public WebFinderMetadata(final String identifier, final JavaType aspectName,
  ...
  private MethodMetadataBuilder getFinderFormMethod(final FinderMetadataDetails finder) {
    ...
    final JavaSymbolName finderFormMethodName =
        new JavaSymbolName(finder.getFinderMethodMetadata().getMethodName().getSymbolName()
            + "Form");
    ...
    final List<JavaType> finderParameterTypes =
        AnnotatedJavaType.convertFromAnnotatedJavaTypes(finder.getFinderMethodMetadata()
            .getParameterTypes());
    ...
    for (final JavaType finderParameterType : finderParameterTypes) {
      ...
      if (finderParameterType.isCommonCollectionType()) {
        ...
        bodyBuilder.appendFormalLine("uiModel.addAttribute(\"" + typeMd.getPlural().toLowerCase()
            + "\", java.util.Arrays.asList(" + getShortName(finderParameterType)
            + ".class.getEnumConstants()));");
      } else if (typeMd != null && typeMd.isApplicationType()) {
        javaTypePersistenceMetadataHolder = typeMd.getPersistenceDetails();
      }
      if (typeMd != null && javaTypePersistenceMetadataHolder != null
    ...
    arrayValues.add(new StringAttributeValue(new JavaSymbolName("value"), "find="
        + finder.getFinderMethodMetadata().getMethodName().getSymbolName()
            .replaceFirst("find" + javaTypeMetadataHolder.getPlural(), "")));
    arrayValues.add(new StringAttributeValue(new JavaSymbolName("value"), "form"));
    requestMappingAttributes.add(new ArrayAttributeValue<StringAttributeValue>(new JavaSymbolName(
        "params"), arrayValues));
    ...
    final MethodMetadataBuilder methodBuilder =
        new MethodMetadataBuilder(getId(), Modifier.PUBLIC, finderFormMethodName, JavaType.STRING,
            AnnotatedJavaType.convertFromJavaTypes(methodParameterTypes), methodParameterNames,
            bodyBuilder);
  ...
  private MethodMetadataBuilder getFinderMethod(final FinderMetadataDetails finderMetadataDetails) {
    ...
    final JavaSymbolName finderMethodName =
        new JavaSymbolName(finderMetadataDetails.getFinderMethodMetadata().getMethodName()
            .getSymbolName());
    ...
    for (final FieldMetadata field : finderMetadataDetails.getFinderMethodParamFields()) {
      ...
      final List<AnnotationAttributeValue<?>> attributes =
          new ArrayList<AnnotationAttributeValue<?>>();
      attributes.add(new StringAttributeValue(new JavaSymbolName("value"), uncapitalize(fieldName
          .getSymbolName())));
    ...
    parameterTypes.add(new AnnotatedJavaType(new JavaType(Integer.class.getName()),
        firstResultAnnotation.build()));
    parameterTypes.add(new AnnotatedJavaType(new JavaType(Integer.class.getName()),
        maxResultAnnotation.build()));
    parameterTypes.add(new AnnotatedJavaType(new JavaType(String.class.getName()),
    ...
    bodyBuilder
        .appendFormalLine("uiModel.addAttribute(\""
    ...
    bodyBuilder.appendFormalLine("float nrOfPages = (float) " + getShortName(formBackingType) + "."
        + countMethodName + "(" + methodParamsString + ") / sizeNo;");
    bodyBuilder
    ...
    bodyBuilder.appendFormalLine("uiModel.addAttribute(\""
        + javaTypeMetadataHolder.getPlural().toLowerCase() + "\", " + getShortName(formBackingType)
        + "." + finderMetadataDetails.getFinderMethodMetadata().getMethodName().getSymbolName()
    ...
    final MethodMetadataBuilder methodBuilder =
        new MethodMetadataBuilder(getId(), Modifier.PUBLIC, finderMethodName, JavaType.STRING,
