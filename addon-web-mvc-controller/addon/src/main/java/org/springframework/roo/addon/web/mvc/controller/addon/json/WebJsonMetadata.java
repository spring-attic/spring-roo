package org.springframework.roo.addon.web.mvc.controller.addon.json;
...
public class WebJsonMetadata extends AbstractItdTypeDetailsProvidingMetadataItem {
  ...
  private static final JavaType RESPONSE_ENTITY_STRING = new JavaType(
      RESPONSE_ENTITY.getFullyQualifiedTypeName(), 0, DataType.TYPE, null,
   ...
   */
  public WebJsonMetadata(final String identifier, final JavaType aspectName,
      ...
      final Set<FinderMetadataDetails> finderDetails, final JsonMetadata jsonMetadata,
      final boolean introduceLayerComponents) {
    super(identifier, aspectName, governorPhysicalTypeMetadata);
    Validate.isTrue(isValid(identifier), "Metadata identification string '%s' is invalid",
        identifier);
    Validate.notNull(annotationValues, "Annotation values required");
    Validate.notNull(persistenceAdditions, "Persistence additions required");
    Validate.notNull(finderDetails, "Set of dynamic finder methods cannot be null");
    ...
    jsonBeanName = JavaSymbolName.getReservedWordSafeName(jsonEnabledType).getSymbolName();

    this.introduceLayerComponents = introduceLayerComponents;
    this.jsonMetadata = jsonMetadata;

    final MemberTypeAdditions findMethod = persistenceAdditions.get(FIND_METHOD);
    builder.addMethod(getShowJsonMethod(identifierField, findMethod));

    final MemberTypeAdditions findAllMethod = persistenceAdditions.get(FIND_ALL_METHOD);
    builder.addMethod(getListJsonMethod(findAllMethod));

    final MemberTypeAdditions persistMethod = persistenceAdditions.get(PERSIST_METHOD);
    builder.addMethod(getCreateFromJsonMethod(persistMethod));
    builder.addMethod(getCreateFromJsonArrayMethod(persistMethod));

    final MemberTypeAdditions mergeMethod = persistenceAdditions.get(MERGE_METHOD);
    ...
    final MemberTypeAdditions removeMethod = persistenceAdditions.get(REMOVE_METHOD);
    builder.addMethod(getDeleteFromJsonMethod(removeMethod, identifierField, findMethod));
  ...
  private void closeTry(InvocableMemberBodyBuilder bodyBuilder, String responseEntityShortName,
      String httpStatusShortName) {
  ...
  private MethodMetadataBuilder getCreateFromJsonArrayMethod(final MemberTypeAdditions persistMethod) {
    if (StringUtils.isBlank(annotationValues.getCreateFromJsonArrayMethod())
        || persistMethod == null) {
      return null;
    }
    final JavaSymbolName methodName =
        new JavaSymbolName(annotationValues.getCreateFromJsonArrayMethod());
    if (governorHasMethodWithSameName(methodName)) {
      return null;
    }

    final JavaSymbolName fromJsonArrayMethodName = jsonMetadata.getFromJsonArrayMethodName();

    final AnnotationMetadataBuilder requestBodyAnnotation =
        new AnnotationMetadataBuilder(REQUEST_BODY);
    final List<AnnotatedJavaType> parameterTypes =
        Arrays.asList(new AnnotatedJavaType(JavaType.STRING, requestBodyAnnotation.build()));
    ...
    requestMappingAttributes
        .add(new StringAttributeValue(new JavaSymbolName("value"), "/jsonArray"));
    requestMappingAttributes.add(new EnumAttributeValue(new JavaSymbolName("method"),
        new EnumDetails(REQUEST_METHOD, new JavaSymbolName("POST"))));
    requestMappingAttributes.add(new StringAttributeValue(new JavaSymbolName("headers"),
    ...
    openTry(bodyBuilder);
    bodyBuilder.appendFormalLine("for (" + jsonEnabledTypeShortName + " " + jsonBeanName + ": "
        + jsonEnabledTypeShortName + "." + fromJsonArrayMethodName.getSymbolName() + "(json)) {");
    bodyBuilder.indent();
    bodyBuilder.appendFormalLine(persistMethod.getMethodCall() + ";");
    bodyBuilder.indentRemove();
    bodyBuilder.appendFormalLine("}");
    bodyBuilder.appendFormalLine("return new ResponseEntity<String>(headers, "
        + getShortName(HTTP_STATUS) + ".CREATED);");
    closeTry(bodyBuilder, getShortName(RESPONSE_ENTITY), getShortName(HTTP_STATUS));
    ...
    final MethodMetadataBuilder methodBuilder =
        new MethodMetadataBuilder(getId(), PUBLIC, methodName, RESPONSE_ENTITY_STRING,
            parameterTypes, parameterNames, bodyBuilder);
    methodBuilder.setAnnotations(annotations);
    return methodBuilder;
  }

  private MethodMetadataBuilder getCreateFromJsonMethod(final MemberTypeAdditions persistMethod) {
    if (StringUtils.isBlank(annotationValues.getCreateFromJsonMethod()) || persistMethod == null) {
      return null;
    }
    final JavaSymbolName methodName =
        new JavaSymbolName(annotationValues.getCreateFromJsonMethod());
    if (governorHasMethodWithSameName(methodName)) {
      return null;
    }

    final JavaSymbolName fromJsonMethodName = jsonMetadata.getFromJsonMethodName();
    ...
    final List<JavaSymbolName> parameterNames =
        Arrays.asList(new JavaSymbolName("json"), new JavaSymbolName("uriBuilder"));
    ...
    bodyBuilder.appendFormalLine("headers.add(\"Content-Type\", \"" + CONTENT_TYPE + "\");");
    openTry(bodyBuilder);
    bodyBuilder.appendFormalLine(jsonEnabledTypeShortName + " " + jsonBeanName + " = "
        + jsonEnabledTypeShortName + "." + fromJsonMethodName.getSymbolName() + "(json);");
    bodyBuilder.appendFormalLine(persistMethod.getMethodCall() + ";");
    bodyBuilder
        .appendFormalLine("RequestMapping a = (RequestMapping) getClass().getAnnotation(RequestMapping.class);");
    bodyBuilder.appendFormalLine("headers.add(\"Location\",uriBuilder.path(a.value()[0]+\"/\"+"
        + jsonBeanName + ".getId().toString()).build().toUriString());");
    bodyBuilder.appendFormalLine("return new ResponseEntity<String>(headers, "
        + getShortName(HTTP_STATUS) + ".CREATED);");
    closeTry(bodyBuilder, getShortName(RESPONSE_ENTITY), getShortName(HTTP_STATUS));
  ...
  private MethodMetadataBuilder getDeleteFromJsonMethod(final MemberTypeAdditions removeMethod,
      final FieldMetadata identifierField, final MemberTypeAdditions findMethod) {
    if (StringUtils.isBlank(annotationValues.getDeleteFromJsonMethod()) || removeMethod == null
        || identifierField == null || findMethod == null) {
      return null;
    }
    final JavaSymbolName methodName =
        new JavaSymbolName(annotationValues.getDeleteFromJsonMethod());
    if (governorHasMethodWithSameName(methodName)) {
      return null;
    }
    ...
    final List<AnnotationAttributeValue<?>> attributes =
        new ArrayList<AnnotationAttributeValue<?>>();
    attributes.add(new StringAttributeValue(new JavaSymbolName("value"), identifierField
        .getFieldName().getSymbolName()));
    final AnnotationMetadataBuilder pathVariableAnnotation =
        new AnnotationMetadataBuilder(PATH_VARIABLE, attributes);

    final List<AnnotatedJavaType> parameterTypes =
        Arrays.asList(new AnnotatedJavaType(identifierField.getFieldType(), pathVariableAnnotation
            .build()));
    ...
    requestMappingAttributes.add(new EnumAttributeValue(new JavaSymbolName("method"),
        new EnumDetails(REQUEST_METHOD, new JavaSymbolName("DELETE"))));
    requestMappingAttributes.add(new StringAttributeValue(new JavaSymbolName("headers"),
        "Accept=application/json"));
    final AnnotationMetadataBuilder requestMapping =
        new AnnotationMetadataBuilder(REQUEST_MAPPING, requestMappingAttributes);
    ...
    bodyBuilder.indent();
    bodyBuilder.appendFormalLine("return new " + getShortName(RESPONSE_ENTITY)
        + "<String>(headers, " + getShortName(HTTP_STATUS) + ".NOT_FOUND);");
    bodyBuilder.indentRemove();
    bodyBuilder.appendFormalLine("}");
  ...
  private MethodMetadataBuilder getJsonFindMethod(final FinderMetadataDetails finderDetails,
      final String plural) {
    if (finderDetails == null || jsonMetadata.getToJsonArrayMethodName() == null) {
      return null;
    }
    final JavaSymbolName finderMethodName =
        new JavaSymbolName("json"
            + StringUtils.capitalize(finderDetails.getFinderMethodMetadata().getMethodName()
                .getSymbolName()));
    ...
    for (final FieldMetadata field : finderDetails.getFinderMethodParamFields()) {
      ...
      final List<AnnotationAttributeValue<?>> attributes =
          new ArrayList<AnnotationAttributeValue<?>>();
      attributes.add(new StringAttributeValue(new JavaSymbolName("value"), StringUtils
      ...
      if (field.getFieldType().equals(JavaType.BOOLEAN_OBJECT)) {
        methodParams.append(field.getFieldName() + " == null ? Boolean.FALSE : "
            + field.getFieldName() + ", ");
      } else {
    ...
    final List<AnnotationAttributeValue<?>> requestMappingAttributes =
        new ArrayList<AnnotationAttributeValue<?>>();
    requestMappingAttributes.add(new StringAttributeValue(new JavaSymbolName("params"), "find="
        + finderDetails.getFinderMethodMetadata().getMethodName().getSymbolName()
    ...
    bodyBuilder.appendFormalLine("headers.add(\"Content-Type\", \"" + CONTENT_TYPE
        + "; charset=utf-8\");");
    bodyBuilder
        .appendFormalLine("return new " + responseEntityShortName + "<String>("
            + jsonEnabledTypeShortName + "."
            + jsonMetadata.getToJsonArrayMethodName().getSymbolName().toString() + "("
            + jsonEnabledTypeShortName + "."
            + finderDetails.getFinderMethodMetadata().getMethodName().getSymbolName() + "("
            + methodParams.toString() + ").getResultList()), headers, " + httpStatusShortName
            + ".OK);");
    closeTry(bodyBuilder, responseEntityShortName, httpStatusShortName);

    final MethodMetadataBuilder methodBuilder =
        new MethodMetadataBuilder(getId(), PUBLIC, finderMethodName, RESPONSE_ENTITY_STRING,
            parameterTypes, newParamNames, bodyBuilder);
    methodBuilder.setAnnotations(annotations);
    return methodBuilder;
  }

  private MethodMetadataBuilder getListJsonMethod(final MemberTypeAdditions findAllMethod) {
    if (StringUtils.isBlank(annotationValues.getListJsonMethod()) || findAllMethod == null) {
      return null;
    }
    final JavaSymbolName methodName = new JavaSymbolName(annotationValues.getListJsonMethod());
    if (governorHasMethodWithSameName(methodName)) {
      return null;
    }

    final JavaSymbolName toJsonArrayMethodName = jsonMetadata.getToJsonArrayMethodName();
    ...
    openTry(bodyBuilder);
    final JavaType list =
        new JavaType(List.class.getName(), 0, DataType.TYPE, null, Arrays.asList(jsonEnabledType));
    bodyBuilder.appendFormalLine(getShortName(list) + " result = " + findAllMethod.getMethodCall()
        + ";");
    bodyBuilder.appendFormalLine("return new " + responseEntityShortName + "<String>("
        + jsonEnabledTypeShortName + "." + toJsonArrayMethodName.getSymbolName()
        + "(result), headers, " + httpStatusShortName + ".OK);");
    closeTry(bodyBuilder, responseEntityShortName, httpStatusShortName);
    ...
    final MethodMetadataBuilder methodBuilder =
        new MethodMetadataBuilder(getId(), PUBLIC, methodName, RESPONSE_ENTITY_STRING, bodyBuilder);
    methodBuilder.setAnnotations(annotations);
    return methodBuilder;
  }
  ...
  private MethodMetadataBuilder getShowJsonMethod(final FieldMetadata identifierField,
      final MemberTypeAdditions findMethod) {
    if (StringUtils.isBlank(annotationValues.getShowJsonMethod()) || identifierField == null
        || findMethod == null) {
      return null;
    }
    final JavaSymbolName methodName = new JavaSymbolName(annotationValues.getShowJsonMethod());
    if (governorHasMethodWithSameName(methodName)) {
      return null;
    }

    final JavaSymbolName toJsonMethodName = jsonMetadata.getToJsonMethodName();
    ...
    final List<AnnotatedJavaType> parameterTypes =
        Arrays.asList(new AnnotatedJavaType(identifierField.getFieldType(), pathVariableAnnotation
            .build()));
    final List<JavaSymbolName> parameterNames =
        Arrays.asList(new JavaSymbolName(identifierField.getFieldName().getSymbolName()));
    ...
    bodyBuilder.appendFormalLine("if (" + jsonBeanName + " == null) {");
    bodyBuilder.indent();
    bodyBuilder.appendFormalLine("return new " + responseEntityShortName + "<String>(headers, "
        + httpStatusShortName + ".NOT_FOUND);");
    bodyBuilder.indentRemove();
    bodyBuilder.appendFormalLine("}");
    bodyBuilder.appendFormalLine("return new " + responseEntityShortName + "<String>("
        + jsonBeanName + "." + toJsonMethodName.getSymbolName() + "(), headers, "
  ...
  private MethodMetadataBuilder getUpdateFromJsonArrayMethod(final MemberTypeAdditions mergeMethod) {
    ...
    final JavaSymbolName methodName =
        new JavaSymbolName(annotationValues.getUpdateFromJsonArrayMethod());
    if (governorHasMethodWithSameName(methodName)) {
      return null;
    }
    ...
    requestMappingAttributes
        .add(new StringAttributeValue(new JavaSymbolName("value"), "/jsonArray"));
    requestMappingAttributes.add(new EnumAttributeValue(new JavaSymbolName("method"),
        new EnumDetails(REQUEST_METHOD, new JavaSymbolName("PUT"))));
    requestMappingAttributes.add(new StringAttributeValue(new JavaSymbolName("headers"),
        "Accept=application/json"));
    final AnnotationMetadataBuilder requestMapping =
        new AnnotationMetadataBuilder(REQUEST_MAPPING, requestMappingAttributes);
  ...
  private MethodMetadataBuilder getUpdateFromJsonMethod(final FieldMetadata identifierField,
      final MemberTypeAdditions mergeMethod) {
    if (StringUtils.isBlank(annotationValues.getUpdateFromJsonMethod()) || mergeMethod == null) {
      return null;
    }
    final JavaSymbolName methodName =
        new JavaSymbolName(annotationValues.getUpdateFromJsonMethod());
    if (governorHasMethodWithSameName(methodName)) {
      return null;
    }
    ...
    final List<AnnotationAttributeValue<?>> attributes =
        new ArrayList<AnnotationAttributeValue<?>>();
    attributes.add(new StringAttributeValue(new JavaSymbolName("value"), identifierFieldName
        .getSymbolName()));
    final AnnotationMetadataBuilder pathVariableAnnotation =
        new AnnotationMetadataBuilder(PATH_VARIABLE, attributes);
    ...
    final List<AnnotatedJavaType> parameterTypes =
        Arrays.asList(new AnnotatedJavaType(JavaType.STRING, requestBodyAnnotation.build()),
            new AnnotatedJavaType(identifierField.getFieldType(), pathVariableAnnotation.build()));
    final List<JavaSymbolName> parameterNames =
        Arrays.asList(new JavaSymbolName("json"), identifierFieldName);
