...
public class ConversionServiceMetadata extends AbstractItdTypeDetailsProvidingMetadataItem {
	...
	private static final JavaSymbolName INSTALL_LABEL_CONVERTERS = new JavaSymbolName("installLabelConverters");
	...
	public ConversionServiceMetadata(final String identifier, final JavaType aspectName, final PhysicalTypeMetadata governorPhysicalTypeMetadata, final Map<JavaType, MemberTypeAdditions> findMethods, final Map<JavaType, JavaType> idTypes, final Set<JavaType> relevantDomainTypes, final Map<JavaType, Map<Object, JavaSymbolName>> compositePrimaryKeyTypes, final Map<JavaType, List<MethodMetadata>> toStringMethods) {
		...
		Assert.notNull(relevantDomainTypes, "List of relevant domain types required");
	...
	private MethodMetadataBuilder getInstallLabelConvertersMethod() {
		...
		for (final JavaType formBackingObject : sortedRelevantDomainTypes) {
			...
			builder.addMethod(getToStringConverterMethod(formBackingObject, toIdMethodName, toStringMethods.get(formBackingObject)));
			...
			if (!idTypes.get(formBackingObject).equals(JavaType.STRING)) {
				...
				builder.addMethod(getStringToTypeConverterMethod(formBackingObject, stringToTypeMethodName, idTypes.get(formBackingObject)));
				bodyBuilder.appendFormalLine("registry.addConverter(" + stringToTypeMethodName.getSymbolName() + "());");
			}
		}
		...
		for (final Entry<JavaType, Map<Object, JavaSymbolName>> entry : compositePrimaryKeyTypes.entrySet()) {
			...
			final MethodMetadataBuilder jsonToConverterMethod = getJsonToConverterMethod(targetType, jsonMethodNames.get(CustomDataJsonTags.FROM_JSON_METHOD));
			builder.addMethod(jsonToConverterMethod);
			bodyBuilder.appendFormalLine("registry.addConverter(" + jsonToConverterMethod.getMethodName().getSymbolName() + "());");

			final MethodMetadataBuilder toJsonConverterMethod = getToJsonConverterMethod(targetType, jsonMethodNames.get(CustomDataJsonTags.TO_JSON_METHOD));
			builder.addMethod(toJsonConverterMethod);
			bodyBuilder.appendFormalLine("registry.addConverter(" + toJsonConverterMethod.getMethodName().getSymbolName() + "());");
		}
		...
		return new MethodMetadataBuilder(getId(), Modifier.PUBLIC, INSTALL_LABEL_CONVERTERS, JavaType.VOID_PRIMITIVE, AnnotatedJavaType.convertFromJavaTypes(parameterType), parameterNames, bodyBuilder);
	}

	private MethodMetadataBuilder getJsonToConverterMethod(final JavaType targetType, final JavaSymbolName jsonMethodName) {
		final JavaSymbolName methodName = new JavaSymbolName("getJsonTo" + targetType.getSimpleTypeName() + CONVERTER);
		...
		bodyBuilder.indent();
		bodyBuilder.appendFormalLine("return " + typeName + "." + jsonMethodName.getSymbolName() + "(new String(" + base64Name + ".decodeBase64(encodedJson)));");
	...
	private MethodMetadataBuilder getToJsonConverterMethod(final JavaType targetType, final JavaSymbolName jsonMethodName) {
		JavaSymbolName methodName = new JavaSymbolName("get" + targetType.getSimpleTypeName() + "ToJsonConverter");
		if (governorHasMethod(methodName)) {
			return null;
		}
		...
		bodyBuilder.indent();
		bodyBuilder.appendFormalLine("return " + base64Name + ".encodeBase64URLSafeString(" + targetTypeName + "." + jsonMethodName.getSymbolName() + "().getBytes());");
		bodyBuilder.indentRemove();
		bodyBuilder.appendFormalLine("}");
		bodyBuilder.indentRemove();
		bodyBuilder.appendFormalLine("};");
	...
	private MethodMetadataBuilder getToStringConverterMethod(final JavaType targetType, final JavaSymbolName methodName, List<MethodMetadata> toStringMethods) {
		if (governorHasMethod(methodName)) {
			return null;
		}
	 ...
	 */
	private MethodMetadataBuilder getStringToTypeConverterMethod(final JavaType targetType, final JavaSymbolName methodName, final JavaType idType) {
		if (governorHasMethod(methodName)) {
			return null;
		}
	...
	private MethodMetadataBuilder getAfterPropertiesSetMethod() {
		...
		return new MethodMetadataBuilder(getId(), Modifier.PUBLIC, methodName, JavaType.VOID_PRIMITIVE, bodyBuilder);
	}
}