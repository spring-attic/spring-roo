...
public class CreatorOperationsImpl implements CreatorOperations {
  ...
  private static final String ICON_SET_URL =
  ...
  @Reference
  private UrlInputStreamService httpService;
  ...
  public void createRooAddonSuite(final JavaPackage topLevelPackage, final String description,
      String projectName) {
  ...
  public void createAdvancedAddon(final JavaPackage topLevelPackage, final String description,
      final String projectName, String folder) {
    ...
    install("Metadata.java", topLevelPackage, Path.SRC_MAIN_JAVA, Type.ADVANCED, projectName,
        folder);
    install("MetadataProvider.java", topLevelPackage, Path.SRC_MAIN_JAVA, Type.ADVANCED,
        projectName, folder);
    install("RooAnnotation.java", topLevelPackage, Path.SRC_MAIN_JAVA, Type.ADVANCED, projectName,
        folder);
    install("configuration.xml", topLevelPackage, Path.SRC_MAIN_RESOURCES, Type.ADVANCED,
        projectName, folder);
  ...
  private void createObrFile(String topLevelPackageName, Type type, String folder) {
    ...
    if (folder != null) {
      ...
      fileManager.createFile(pathResolver.getIdentifier(rootPath, folder + "/"
          + "src/main/resources/obr.xml"));
    } else {
      LogicalPath rootPath = LogicalPath.getInstance(Path.ROOT, "");
    ...
    if (capabilityElement != null) {
      ...
      if (type.equals(Type.ADVANCED)) {
        ...
        String commandInit =
            topLevelPackageName.substring(topLevelPackageName.lastIndexOf(".") + 1).toLowerCase();
      ...
      if (folder != null) {
        ...
        XmlUtils.writeXml(
            fileManager.updateFile(
                pathResolver.getIdentifier(rootPath, folder + "/" + "src/main/resources/obr.xml"))
        ...
        XmlUtils.writeXml(
            fileManager.updateFile(
                pathResolver.getIdentifier(rootPath, "src/main/resources/obr.xml"))
                .getOutputStream(), docXml);
      }
    }
  ...
  public void createI18nAddon(final JavaPackage topLevelPackage, String language,
      final Locale locale, final File messageBundle, final File flagGraphic, String description,
    ...
    if (StringUtils.isBlank(language)) {
      ...
      final InputStream inputStream =
          FileUtils.getInputStream(getClass(), Type.I18N.name().toLowerCase() + "/iso3166.txt");
      try {
    ...
    final String packagePath =
        topLevelPackage.getFullyQualifiedPackageName().replace('.', separatorChar);

    if (StringUtils.isBlank(description)) {
      description = languageName + " language support for Spring Roo Web MVC JSP Scaffolding";
    }
    if (!description.contains("#mvc") || !description.contains("#localization")
        || !description.contains("locale:")) {
      description =
          description + "; #mvc,#localization,locale:" + locale.getCountry().toLowerCase();
    }
    ...
    try {
      outputStream =
          fileManager.createFile(
              pathResolver.getFocusedIdentifier(Path.SRC_MAIN_RESOURCES, packagePath
                  + separatorChar + messageBundle.getName())).getOutputStream();
      org.apache.commons.io.FileUtils.copyFile(messageBundle, outputStream);
      if (flagGraphic != null) {
        outputStream =
            fileManager.createFile(
                pathResolver.getFocusedIdentifier(Path.SRC_MAIN_RESOURCES, packagePath
                    + separatorChar + flagGraphic.getName())).getOutputStream();
    ...
    final String destinationFile =
        pathResolver.getFocusedIdentifier(Path.SRC_MAIN_JAVA, packagePath + separatorChar
            + languageName + "Language.java");

    if (!fileManager.exists(destinationFile)) {
      final InputStream templateInputStream =
          FileUtils.getInputStream(getClass(), Type.I18N.name().toLowerCase()
              + "/Language.java-template");
      try {
        // Read template and insert the user's package
        String input = IOUtils.toString(templateInputStream);
        input =
            input.replace("__TOP_LEVEL_PACKAGE__", topLevelPackage.getFullyQualifiedPackageName());
   ...
   * @param folder  
   */
  private void createProject(final JavaPackage topLevelPackage, final Type type,
      final String description, String projectName, String folder) {
    ...
    if (type.equals(Type.PARENT)) {
      XmlUtils.findRequiredElement("/project/artifactId", root).setTextContent(
          topLevelPackage.getFullyQualifiedPackageName() + ".root");
    } else if (type.name().contains("CHILD") || type.equals(Type.SUITE)) {
      XmlUtils.findRequiredElement("/project/parent/artifactId", root).setTextContent(
          topLevelPackage.getFullyQualifiedPackageName() + ".osgi.bundles");
      XmlUtils.findRequiredElement("/project/artifactId", root).setTextContent(
          topLevelPackage.getFullyQualifiedPackageName() + "." + folder);
      ...
      XmlUtils.findRequiredElement("/project/parent/artifactId", root).setTextContent(
          topLevelPackage.getFullyQualifiedPackageName() + ".root");
      XmlUtils.findRequiredElement("/project/artifactId", root).setTextContent(
          topLevelPackage.getFullyQualifiedPackageName() + ".repository");
    } else {
      XmlUtils.findRequiredElement("/project/artifactId", root).setTextContent(
          topLevelPackage.getFullyQualifiedPackageName());
    }
    if (type.name().contains("CHILD") || type.equals(Type.OSGIBUNDLES) || type.equals(Type.SUITE)
        || type.equals(Type.REPOSITORY)) {
    ...
    if (type.equals(Type.SUITE)) {
      writeTextFile("MANIFEST.MF",
          generateManifestContent(topLevelPackage, description, projectName), folder
              + "/src/main/esa/META-INF");
    }
    ...
    writeTextFile("legal" + separatorChar + "LICENSE.TXT", "Your license goes here", folder);
    fileManager.scan();
  }
  ...
  private void writeAssemblyFile(JavaPackage topLevelPackage, String templateName,
    ...
    if (includes != null) {
      ...
      includeAdvancedElement.setTextContent(topLevelPackage.getFullyQualifiedPackageName() + ":"
          + topLevelPackage.getFullyQualifiedPackageName() + ".addon-advanced");

      // Adding addon-simple include
      Element includeSimpleElement = assemblyDoc.createElement("include");
      includeSimpleElement.setTextContent(topLevelPackage.getFullyQualifiedPackageName() + ":"
          + topLevelPackage.getFullyQualifiedPackageName() + ".addon-simple");

      // Adding roo-addon-suite include
      Element includeSuiteElement = assemblyDoc.createElement("include");
      includeSuiteElement.setTextContent(topLevelPackage.getFullyQualifiedPackageName() + ":"
          + topLevelPackage.getFullyQualifiedPackageName() + ".roo-addon-suite");
    ...
    LogicalPath rootPath = LogicalPath.getInstance(Path.ROOT, "");
    MutableFile assemblyFile =
        fileManager.createFile(pathResolver.getIdentifier(rootPath,
            "src/main/assembly/repo-assembly.xml"));

    XmlUtils.writeXml(assemblyFile.getOutputStream(), assemblyDoc);

  }

  private String generateManifestContent(JavaPackage topLevelPackage, String description,
      String projectName) {
  ...
  private void addAddonsDependencies(Document pom, String folder, JavaPackage topLevelPackage) {
    ...
    String pomRooAddonSuite =
        projectOperations.getPathResolver().getIdentifier(Path.ROOT.getModulePathId(""),
            folder + "/pom.xml");

    Validate.isTrue(fileManager.exists(pomRooAddonSuite), folder + "/pom.xml not found");
    ...
    Element dependenciesElement = XmlUtils.findFirstElement("dependencies", document);
    ...
    if (dependenciesElement != null) {
      ...
      artifactIdAdvancedElement.setTextContent(topLevelPackage.getFullyQualifiedPackageName()
          + ".addon-advanced");
      ...
      artifactIdElement.setTextContent(topLevelPackage.getFullyQualifiedPackageName()
          + ".addon-simple");
      ...
      XmlUtils.writeXml(fileManager.updateFile(pomRooAddonSuite).getOutputStream(), docXml);

    }

  }

  public void createSimpleAddon(final JavaPackage topLevelPackage, final String description,
      final String projectName, String folder) {
    Validate.notNull(topLevelPackage, "Top Level Package required");
    ...
    install("Commands.java", topLevelPackage, Path.SRC_MAIN_JAVA, Type.SIMPLE, projectName, folder);
    ...
    install("PropertyName.java", topLevelPackage, Path.SRC_MAIN_JAVA, Type.SIMPLE, projectName,
        folder);
    install("info.tagx", topLevelPackage, Path.SRC_MAIN_RESOURCES, Type.SIMPLE, projectName, folder);
    install("show.tagx", topLevelPackage, Path.SRC_MAIN_RESOURCES, Type.SIMPLE, projectName, folder);

    createObrFile(topLevelPackage.getFullyQualifiedPackageName(), Type.SIMPLE, folder);
  }

  public void createWrapperAddon(final JavaPackage topLevelPackage, final String groupId,
      final String artifactId, final String version, final String vendorName,
      final String lincenseUrl, final String docUrl, final String osgiImports,
    ...
    XmlUtils.findRequiredElement("/project/artifactId", root).setTextContent(
        wrapperGroupId + "." + artifactId);
    XmlUtils.findRequiredElement("/project/version", root).setTextContent(version + ".0001");
    XmlUtils.findRequiredElement("/project/dependencies/dependency/groupId", root).setTextContent(
        groupId);
    XmlUtils.findRequiredElement("/project/dependencies/dependency/artifactId", root)
        .setTextContent(artifactId);
    XmlUtils.findRequiredElement("/project/dependencies/dependency/version", root).setTextContent(
        version);
    XmlUtils.findRequiredElement("/project/properties/pkgArtifactId", root).setTextContent(
        artifactId);
  ...
  private void install(final String targetFilename, final JavaPackage topLevelPackage,
      final Path path, final Type type, String projectName, String folder) {
    if (StringUtils.isBlank(projectName)) {
      projectName = topLevelPackage.getFullyQualifiedPackageName().replace(".", "-");
    }
    ...
    if (targetFilename.endsWith(".java")) {
      destinationFile =
          path.getDefaultLocation()
              ...
              + packagePath
              + separatorChar
              + StringUtils.capitalize(topLevelPackageName.substring(topLevelPackageName
                  .lastIndexOf(".") + 1)) + targetFilename;
    } else {
      destinationFile =
          path.getDefaultLocation() + "/" + packagePath + separatorChar + targetFilename;
    }
    ...
    if (!fileManager.exists(destinationFile)) {
      ...
      try {
        ...
        input =
            input.replace("__TOP_LEVEL_PACKAGE__", topLevelPackage.getFullyQualifiedPackageName());
        input =
            input.replace("__APP_NAME__", StringUtils.capitalize(topLevelPackageName
                .substring(topLevelPackageName.lastIndexOf(".") + 1)));
        input =
            input.replace("__APP_NAME_LWR_CASE__",
                topLevelPackageName.substring(topLevelPackageName.lastIndexOf(".") + 1)
                    .toLowerCase());
        ...
        if (folder != null) {
          ...
          LogicalPath rootPath = LogicalPath.getInstance(Path.ROOT, "");
          mutableFile =
              fileManager.createFile(pathResolver.getIdentifier(rootPath, destinationFile));
        }
    ...
    try {
      ...
      while ((entry = zis.getNextEntry()) != null) {
        if (entry.getName().equals(expectedEntryName)) {
          final MutableFile target =
              fileManager.createFile(pathResolver.getFocusedIdentifier(Path.SRC_MAIN_RESOURCES,
  ...
  private void writePomFile(final Document pom, String folder) {
    ...
    if (folder != null) {
      LogicalPath rootPath = LogicalPath.getInstance(Path.ROOT, "");
      pomFile =
          fileManager.createFile(pathResolver.getIdentifier(rootPath, folder + "/" + POM_XML));
    } else {
      LogicalPath rootPath = LogicalPath.getInstance(Path.ROOT, "");
