...
public class CloudFoundrySessionImplTest {
	...
	public void testLoginWithNonBlankEmailNonBlankPasswordAndCustomUrl() {
		// Set up
		final CloudCredentials credentials = new CloudCredentials(EMAIL, PASSWORD, CUSTOM_CLOUD_URL);
		final UaaAwareAppCloudClient mockClient = mock(UaaAwareAppCloudClient.class);
		when(mockAppCloudClientFactory.getUaaAwareInstance(credentials)).thenReturn(mockClient);
	...
	public void testLoginWithBlankEmailAndNoStoredEmail() {
		// Set up
		final UaaAwareAppCloudClient mockClient = mock(UaaAwareAppCloudClient.class);
		when(mockPreferences.getStoredEmails(CUSTOM_CLOUD_URL)).thenReturn(Collections.<String>emptyList());
	...
	public void testLoginWithBlankEmailAndStoredEmail() {
		// Set up
		when(mockPreferences.getStoredEmails(CUSTOM_CLOUD_URL)).thenReturn(Collections.singletonList(EMAIL));
		final CloudCredentials credentials = new CloudCredentials(EMAIL, PASSWORD, CUSTOM_CLOUD_URL);
		final UaaAwareAppCloudClient mockClient = mock(UaaAwareAppCloudClient.class);
		when(mockAppCloudClientFactory.getUaaAwareInstance(credentials)).thenReturn(mockClient);
	...
	public void testLoginWithBlankPasswordAndNoStoredPassword() {
		// Set up
		final UaaAwareAppCloudClient mockClient = mock(UaaAwareAppCloudClient.class);
		when(mockPreferences.getStoredPassword(CUSTOM_CLOUD_URL, EMAIL)).thenReturn(null);
	...
	public void testLoginWithBlankPasswordAndStoredPassword() {
		// Set up
		when(mockPreferences.getStoredPassword(CUSTOM_CLOUD_URL, EMAIL)).thenReturn(PASSWORD);
