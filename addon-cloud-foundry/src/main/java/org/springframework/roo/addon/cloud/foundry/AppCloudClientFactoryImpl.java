...
public class AppCloudClientFactoryImpl implements AppCloudClientFactory {
	...
	private Product product = VersionHelper.getProduct(CLOUD_FOUNDRY_PRODUCT_NAME, DEFAULT_BUNDLE_VERSION);

	protected void activate(final ComponentContext context) {
		this.product = getCloudFoundryProduct(context.getBundleContext().getBundle().getHeaders());
	}
	...
	private Product getCloudFoundryProduct(final Dictionary<?, ?> bundleHeaders) {
		...
		final String bundleVersion = ObjectUtils.toString(bundleHeaders.get("Bundle-Version"), DEFAULT_BUNDLE_VERSION);
		final String gitCommitHash = ObjectUtils.toString(bundleHeaders.get("Git-Commit-Hash"), null);
		return VersionHelper.getProduct(CLOUD_FOUNDRY_PRODUCT_NAME, bundleVersion, gitCommitHash);
	}
	
	public UaaAwareAppCloudClient getUaaAwareInstance(final CloudCredentials credentials) {
		...
		return new UaaAwareAppCloudClient(product, uaaService, credentials, requestFactory);
	}
}
