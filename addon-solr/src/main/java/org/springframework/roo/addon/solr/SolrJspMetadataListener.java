...
public final class SolrJspMetadataListener implements MetadataProvider, MetadataNotificationListener {
	...
	public void installMvcArtifacts(JavaType javaType, Path path) {
		...
		menuOperations.addMenuItem(new JavaSymbolName(webScaffoldMetadata.getAnnotationValues().getPath()), new JavaSymbolName(entityMetadata.getPlural()), "global.menu.find", "/" + webScaffoldMetadata.getAnnotationValues().getPath() + "?search", "s:");
	}
	...
	private Document getSearchDocument() {
		...
		for (MethodMetadata accessor : beanInfoMetadata.getPublicAccessors()) {
			if(++fieldCounter < 7) {
				...
				FieldMetadata field = beanInfoMetadata.getFieldForPropertyName(BeanInfoMetadata.getPropertyNameForJavaBeanMethod(accessor));
