...
public class PomFactoryImpl implements PomFactory {
	...
	public Pom getInstance(final Element root, final String pomPath, final String moduleName) {
		...
		final List<Repository> pluginRepositories = parseElements(Repository.class, PLUGIN_REPOSITORY_XPATH, root);
		...
		return new Pom(groupId, artifactId, version, packaging, dependencies, parent, modules, pomProperties, name, repositories, pluginRepositories, sourceDirectory, testSourceDirectory, filters, plugins, resources, pomPath, moduleName, paths);
	}
	...
	private Collection<Path> getPaths(final Element root, final String packaging) {
		...
		Assert.notNull(packagingProvider, "No PackagingProvider found with the ID '" + packagingProviderId + "'");
