...
public class MemberDetailsBuilder {
  ...
  static class TypeDetailsBuilder extends
    ...
    public void addDataToConstructor(final ConstructorMetadata replacement,
      ...
      for (final ConstructorMetadataBuilder existingConstructor : getDeclaredConstructors()) {
        if (AnnotatedJavaType
            .convertFromAnnotatedJavaTypes(existingConstructor.getParameterTypes()).equals(
    ...
    public MemberHoldingTypeDetails build() {
      if (existing instanceof ItdTypeDetails) {
        final ItdTypeDetailsBuilder itdBuilder =
            new ItdTypeDetailsBuilder((ItdTypeDetails) existing);
  ...
  private final Map<String, MemberHoldingTypeDetails> memberHoldingTypeDetailsMap =
      new LinkedHashMap<String, MemberHoldingTypeDetails>();
  private final MemberDetails originalMemberDetails;

  private final Map<String, TypeDetailsBuilder> typeDetailsBuilderMap =
      new LinkedHashMap<String, TypeDetailsBuilder>();
  ...
  public MemberDetails build() {
    if (changed) {
      for (final TypeDetailsBuilder typeDetailsBuilder : typeDetailsBuilderMap.values()) {
        memberHoldingTypeDetailsMap.put(typeDetailsBuilder.getDeclaredByMetadataId(),
            typeDetailsBuilder.build());
      }
      return new MemberDetailsImpl(new ArrayList<MemberHoldingTypeDetails>(
  ...
  private void doModification(final MethodMetadata method, final CustomData customData) {
    ...
    if (memberHoldingTypeDetails != null) {
      final MethodMetadata matchedMethod =
          memberHoldingTypeDetails.getMethod(method.getMethodName(),
