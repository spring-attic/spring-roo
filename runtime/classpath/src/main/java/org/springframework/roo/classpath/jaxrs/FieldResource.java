...
public class FieldResource implements ResourceMarker {
    ...
    public JsonObject setupJpa(@FormParam("entityName") String entityName,
            ...
            @FormParam("fieldGroup") String fieldGroup,
            @FormParam("fieldType") String fieldType,
            @FormParam("referencedClass") String referencedClass) {

        // Execute fieldcommand
        // TODO: Replace with field command
        String fieldCommand = constructFieldCommand(entityName, fieldName,
                fieldGroup, fieldType, referencedClass);

        boolean status = getShell().executeCommand(fieldCommand);

        String message = "New field '".concat(fieldName)
                .concat("' created on '").concat(entityName).concat("'!");
        if (!status) {
    ...
    private String constructFieldCommand(String entityName, String fieldName,
        ...
        if ("".equals(fieldGroup)) {
            ...
            sb.append(fieldGroup.toLowerCase()).append(" --type ")
                    .append(fieldType.toLowerCase()).append(" ");
        }
