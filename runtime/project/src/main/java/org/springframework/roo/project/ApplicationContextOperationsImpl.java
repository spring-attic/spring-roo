...
public class ApplicationContextOperationsImpl implements ApplicationContextOperations {
  ...
  protected final static Logger LOGGER = HandlerUtils
      .getLogger(ApplicationContextOperationsImpl.class);
  ...
  public void createMiddleTierApplicationContext(final JavaPackage topLevelPackage,
      final String moduleName) {
    final ProjectMetadata projectMetadata =
        (ProjectMetadata) getMetadataService()
            .get(ProjectMetadata.getProjectIdentifier(moduleName));
    Validate.notNull(projectMetadata, "Project metadata required for module '%s'", moduleName);
    final Document document =
        XmlUtils.readXml(FileUtils.getInputStream(getClass(), "applicationContext-template.xml"));
    final Element root = document.getDocumentElement();
    DomUtils.findFirstElementByName("context:component-scan", root).setAttribute("base-package",
        topLevelPackage.getFullyQualifiedPackageName());
    getFileManager().createOrUpdateTextFileIfRequired(
        getPathResolver().getIdentifier(Path.SPRING_CONFIG_ROOT.getModulePathId(moduleName),
            "applicationContext.xml"), XmlUtils.nodeToString(document), false);
    getFileManager().scan();
  }
