...
public class JavaParserConstructorMetadataBuilder implements Builder<ConstructorMetadata> {
  ...
  public static void addConstructor(final CompilationUnitServices compilationUnitServices,
      final List<BodyDeclaration> members, final ConstructorMetadata constructor,
    ...
    d.setName(PhysicalTypeIdentifier.getJavaType(constructor.getDeclaredByMetadataId())
        .getSimpleTypeName());
    ...
    for (final AnnotatedJavaType constructorParameter : constructor.getParameterTypes()) {
      ...
      if (constructorParameter.getJavaType().isPrimitive()) {
        ...
        final Type finalType =
            JavaParserUtils.getResolvedName(constructorParameter.getJavaType(),
                constructorParameter.getJavaType(), compilationUnitServices);
      ...
      if (bd instanceof ConstructorDeclaration) {
        ...
        if (cd.getParameters().size() == d.getParameters().size()) {
          ...
          final ConstructorMetadata constructorMetadata =
              new JavaParserConstructorMetadataBuilder(constructor.getDeclaredByMetadataId(), cd,
          ...
          if (matchesFully) {
            throw new IllegalStateException("Constructor '" + constructor.getParameterNames()
  ...
  public static JavaParserConstructorMetadataBuilder getInstance(final String declaredByMetadataId,
      final ConstructorDeclaration constructorDeclaration,
      final CompilationUnitServices compilationUnitServices,
      final Set<JavaSymbolName> typeParameterNames) {
    return new JavaParserConstructorMetadataBuilder(declaredByMetadataId, constructorDeclaration,
  ...
  private JavaParserConstructorMetadataBuilder(final String declaredByMetadataId,
      final ConstructorDeclaration constructorDeclaration,
      final CompilationUnitServices compilationUnitServices, Set<JavaSymbolName> typeParameterNames) {
    Validate.notBlank(declaredByMetadataId, "Declared by metadata ID required");
    Validate.notNull(constructorDeclaration, "Constructor declaration is mandatory");
    ...
    modifier = JavaParserUtils.getJdkModifier(constructorDeclaration.getModifiers());
    ...
    final List<TypeParameter> params = constructorDeclaration.getTypeParameters();
    if (params != null) {
      ...
      for (final Parameter p : constructorDeclaration.getParameters()) {
        ...
        if (annotationsList != null) {
          for (final AnnotationExpr candidate : annotationsList) {
            final JavaParserAnnotationMetadataBuilder md =
    ...
    if (constructorDeclaration.getAnnotations() != null) {
      for (final AnnotationExpr annotation : constructorDeclaration.getAnnotations()) {
