...
public class JavaParserClassOrInterfaceTypeDetailsBuilder implements
  ...
  static final String UNSUPPORTED_MESSAGE_PREFIX =
      "Only enum, class and interface files are supported";
  ...
  public static JavaParserClassOrInterfaceTypeDetailsBuilder getInstance(
      ...
      final TypeDeclaration typeDeclaration, final String declaredByMetadataId,
      final JavaType typeName, final MetadataService metadataService,
      final TypeLocationService typeLocationService) {
    return new JavaParserClassOrInterfaceTypeDetailsBuilder(compilationUnit,
        enclosingCompilationUnitServices, typeDeclaration, declaredByMetadataId, typeName,
   ...
   * @param typeLocationService
   */
  private JavaParserClassOrInterfaceTypeDetailsBuilder(final CompilationUnit compilationUnit,
      final CompilationUnitServices enclosingCompilationUnitServices,
      final TypeDeclaration typeDeclaration, final String declaredByMetadataId,
      final JavaType typeName, final MetadataService metadataService,
      final TypeLocationService typeLocationService) {
    ...
    Validate.notNull(typeDeclaration, "Unable to locate the class or interface declaration");
    Validate.notNull(typeName, "Name required");

    // Assign
    this.compilationUnit = compilationUnit;
    compilationUnitServices =
        enclosingCompilationUnitServices == null ? getDefaultCompilationUnitServices()
  ...
  public ClassOrInterfaceTypeDetails build() {
    ...
    Validate.notNull(physicalTypeCategory, "%s (%s for %s)", UNSUPPORTED_MESSAGE_PREFIX,
        typeDeclaration.getClass().getSimpleName(), name);
    ...
    if (compilationUnitPackage.equals(name.getPackage()) != true) {
      String warningStr =
          "[Warning] Compilation unit package '" + compilationUnitPackage
              + "' unexpected for type '" + name.getPackage() + "', it may be a nested class.";
    ...
    for (final ImportDeclaration importDeclaration : imports) {
      if (importDeclaration.getName() instanceof QualifiedNameExpr) {
        final String qualifier =
            ((QualifiedNameExpr) importDeclaration.getName()).getQualifier().toString();
        ...
        final JavaPackage typePackage =
            importDeclaration.isAsterisk() ? new JavaPackage(fullName) : type.getPackage();
        ...
        final ImportMetadataBuilder newImport =
            new ImportMetadataBuilder(declaredByMetadataId, 0, typePackage, type,
      ...
      if (extendsList != null) {
        for (final ClassOrInterfaceType candidate : extendsList) {
          final JavaType javaType =
              JavaParserUtils
                  .getJavaTypeNow(compilationUnitServices, candidate, typeParameterNames);
      ...
      for (final BodyDeclaration member : members) {
          ...
          for (final VariableDeclarator var : castMember.getVariables()) {
            final FieldMetadata field =
                JavaParserFieldMetadataBuilder.getInstance(declaredByMetadataId, castMember, var,
                    compilationUnitServices, typeParameterNames).build();
        ...
        if (member instanceof MethodDeclaration) {
          ...
          final MethodMetadata method =
              JavaParserMethodMetadataBuilder.getInstance(declaredByMetadataId, castMember,
                  compilationUnitServices, typeParameterNames).build();
        ...
        if (member instanceof ConstructorDeclaration) {
          ...
          final ConstructorMetadata constructor =
              JavaParserConstructorMetadataBuilder.getInstance(declaredByMetadataId, castMember,
                  compilationUnitServices, typeParameterNames).build();
        ...
        if (member instanceof TypeDeclaration) {
          ...
          final String innerTypeMetadataId =
              PhysicalTypeIdentifier.createIdentifier(innerType,
                  PhysicalTypeIdentifier.getPath(declaredByMetadataId));
          final ClassOrInterfaceTypeDetails cid =
              new JavaParserClassOrInterfaceTypeDetailsBuilder(compilationUnit,
                  compilationUnitServices, castMember, innerTypeMetadataId, innerType,
