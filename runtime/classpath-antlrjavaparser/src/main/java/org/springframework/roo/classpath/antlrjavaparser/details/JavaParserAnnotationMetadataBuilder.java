...
public class JavaParserAnnotationMetadataBuilder implements Builder<AnnotationMetadata> {
   ...
   */
  public static void addAnnotationToList(final CompilationUnitServices compilationUnitServices,
      final List<AnnotationExpr> annotations, final AnnotationMetadata annotation) {
    ...
    final NameExpr nameToUse =
        JavaParserUtils.importTypeIfRequired(compilationUnitServices.getEnclosingTypeName(),
            compilationUnitServices.getImports(), annotation.getAnnotationType());
    ...
    if (memberValuePairs.isEmpty()) {
      annotationExpression = new MarkerAnnotationExpr(nameToUse);
        ...
        && (memberValuePairs.get(0).getName() == null || "value".equals(memberValuePairs.get(0)
            .getName()))) {
      final Expression toUse =
          JavaParserUtils.importExpressionIfRequired(
    ...
    JavaParserCommentMetadataBuilder.updateCommentsToJavaParser(annotationExpression,
    ...
    if (!memberValuePairs.isEmpty()) {
        ...
        if (memberValuePairs.size() == 1
          ...
          final Expression toUse =
              JavaParserUtils.importExpressionIfRequired(
                  ...
                  compilationUnitServices.getImports(), memberValuePairs.get(0).getValue());
          annotationExpression = new SingleMemberAnnotationExpr(nameToUse, toUse);
          JavaParserCommentMetadataBuilder.updateCommentsToJavaParser(annotationExpression,
              annotation.getCommentStructure());
          annotations.add(annotationExpression);
          ...
          annotationExpression =
              new NormalAnnotationExpr(nameToUse, new ArrayList<MemberValuePair>());
          JavaParserCommentMetadataBuilder.updateCommentsToJavaParser(annotationExpression,
              annotation.getCommentStructure());
          annotations.add(annotationExpression);
        }
      ...
      if (annotationExpression instanceof SingleMemberAnnotationExpr) {
        ...
        if (memberValuePairs.size() == 1 && memberValuePairs.get(0).getName() == null
        ...
        annotationExpression =
            new NormalAnnotationExpr(smae.getName(), new ArrayList<MemberValuePair>());
      ...
      final List<MemberValuePair> annotationPairs =
          ((NormalAnnotationExpr) annotationExpression).getPairs();
      annotationPairs.clear();
      for (final MemberValuePair pair : memberValuePairs) {
        final Expression toUse =
            JavaParserUtils.importExpressionIfRequired(
                compilationUnitServices.getEnclosingTypeName(),
                compilationUnitServices.getImports(), pair.getValue());
        pair.setValue(toUse);
  ...
  private static MemberValuePair convert(final AnnotationAttributeValue<?> value,
    ...
    if (value instanceof CharAttributeValue) {
      ...
      final CharLiteralExpr convertedValue =
          new CharLiteralExpr(new String(new char[] {castValue}));
      return new MemberValuePair(value.getName().getSymbolName(), convertedValue);
    }
    ...
    if (value instanceof EnumAttributeValue) {
      ...
      final FieldAccessExpr convertedValue =
          new FieldAccessExpr(JavaParserUtils.getNameExpr(castValue.getType()
              .getFullyQualifiedTypeName()), castValue.getField().getSymbolName());
      return new MemberValuePair(value.getName().getSymbolName(), convertedValue);
    }
    ...
    if (value instanceof ArrayAttributeValue) {
      final ArrayAttributeValue<AnnotationAttributeValue<?>> castValue =
          (ArrayAttributeValue<AnnotationAttributeValue<?>>) value;
      ...
      return new MemberValuePair(value.getName().getSymbolName(), new ArrayInitializerExpr(
          arrayElements));
    }

    throw new UnsupportedOperationException("Unsupported attribute value '" + value.getName()
        + "' of type '" + value.getClass().getName() + "'");
  }

  public static JavaParserAnnotationMetadataBuilder getInstance(
      final AnnotationExpr annotationExpr, final CompilationUnitServices compilationUnitServices) {
   ...
   * @since 1.2.0
   */
  private JavaParserAnnotationMetadataBuilder(final AnnotationExpr annotationExpr,
    ...
    for (final MemberValuePair p : annotationPairs) {
      final JavaSymbolName annotationName = new JavaSymbolName(p.getName());
      final AnnotationAttributeValue<?> value =
          convert(annotationName, p.getValue(), compilationUnitServices);
  ...
  public AnnotationMetadata build() {
    final AnnotationMetadataBuilder annotationMetadataBuilder =
        new AnnotationMetadataBuilder(annotationType, attributeValues);
  ...
  private AnnotationAttributeValue<?> convert(JavaSymbolName annotationName,
      final Expression expression, final CompilationUnitServices compilationUnitServices) {
    ...
    if (expression instanceof CharLiteralExpr) {
      ...
      Validate.isTrue(value.length() == 1,
          "Expected a char expression, but instead received '%s' for attribute '%s'", value,
          annotationName);
    ...
    if (expression instanceof LongLiteralExpr) {
      ...
      Validate.isTrue(value.toUpperCase().endsWith("L"),
          "Expected long literal expression '%s' to end in 'l' or 'L'", value);
    ...
    if (expression instanceof FieldAccessExpr) {
      ...
      if (scope instanceof FieldAccessExpr) {
        ...
        throw new UnsupportedOperationException("A FieldAccessExpr for '" + field.getScope()
    ...
    if (expression instanceof ArrayInitializerExpr) {
      ...
      final List<AnnotationAttributeValue<?>> arrayElements =
          new ArrayList<AnnotationAttributeValue<?>>();
    ...
    throw new UnsupportedOperationException("Unable to parse annotation attribute '"
        + annotationName + "' due to unsupported annotation expression '"
