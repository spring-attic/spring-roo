...
public final class JavaParserUtils {
   ...
   */
  public static ClassOrInterfaceType getClassOrInterfaceType(final NameExpr nameExpr) {
  ...
  private static ImportDeclaration getImportDeclarationFor(
      final CompilationUnitServices compilationUnitServices, final NameExpr nameExpr) {
   ...
   */
  public static JavaType getJavaType(final CompilationUnitServices compilationUnitServices,
    ...
    if (nameToFind instanceof QualifiedNameExpr) {
      ...
      final NameExpr enclosedBy =
          getNameExpr(compilationUnitServices.getEnclosingTypeName().getSimpleTypeName());
      if (isEqual(qneQualifier, enclosedBy)) {
        // This qualified name expression is simply an inner type
        // reference
        final String name =
            compilationUnitServices.getEnclosingTypeName().getFullyQualifiedTypeName() + "."
                + nameToFind.getName();
      ...
      if (qne.toString().length() > 1 && Character.isUpperCase(qne.toString().charAt(0))) {
        ...
        final ImportDeclaration importDeclaration =
            getImportDeclarationFor(compilationUnitServices, qne.getQualifier());
        if (importDeclaration == null) {
          if (!compilationUnitPackage.getFullyQualifiedPackageName().equals("")) {
            // It was not imported, so let's assume it's in the same
            // package
            return new JavaType(compilationUnitServices.getCompilationUnitPackage()
    ...
    final NameExpr enclosingTypeName =
        getNameExpr(compilationUnitServices.getEnclosingTypeName().getSimpleTypeName());
    ...
    final ImportDeclaration importDeclaration =
        getImportDeclarationFor(compilationUnitServices, nameToFind);
    if (importDeclaration == null) {
  ...
  public static JavaType getJavaType(final CompilationUnitServices compilationUnitServices,
    ...
    if (internalType instanceof ClassOrInterfaceType) {
      ...
      throw new IllegalStateException("The presented type '" + internalType.getClass()
          + "' with value '" + internalType + "' is unsupported by JavaParserUtils");
    }

    final JavaType effectiveType = getJavaTypeNow(compilationUnitServices, cit, typeParameters);
    if (array > 0) {
      return new JavaType(effectiveType.getFullyQualifiedTypeName(), array,
          effectiveType.getDataType(), effectiveType.getArgName(), effectiveType.getParameters());
    }
  ...
  public static JavaType getJavaType(final CompilationUnitServices compilationUnitServices,
      ...
      if (typeParameters != null) {
        ...
        for (final TypeParameter candidate : typeParameters) {
          ...
          if (candidate.getTypeBound() == null) {
            javaType =
                new JavaType(OBJECT.getFullyQualifiedTypeName(), 0, DataType.TYPE,
                    currentTypeParam, null);
          } else {
            final ClassOrInterfaceType cit = candidate.getTypeBound().get(0);
            javaType =
                JavaParserUtils.getJavaTypeNow(compilationUnitServices, cit, locatedTypeParameters);
            javaType =
                new JavaType(javaType.getFullyQualifiedTypeName(), javaType.getArray(),
                    javaType.getDataType(), currentTypeParam, javaType.getParameters());
          }
    ...
    return new JavaType(effectiveType.getFullyQualifiedTypeName(), effectiveType.getArray(),
   ...
   * @return the effective Java type (never null)
   */
  public static JavaType getJavaTypeNow(final CompilationUnitServices compilationUnitServices,
      final ClassOrInterfaceType cit, final Set<JavaSymbolName> typeParameters) {
    Validate.notNull(compilationUnitServices, "Compilation unit services required");
  ...
  public static NameExpr getNameExpr(final AnnotationExpr annotationExpr) {
    ...
    throw new UnsupportedOperationException("Unknown annotation expression type '"
  ...
  public static ClassOrInterfaceType getResolvedName(final JavaType target, final JavaType current,
      final CompilationUnit compilationUnit) {
    final NameExpr nameExpr =
        JavaParserUtils.importTypeIfRequired(target, compilationUnit.getImports(), current);
  ...
  public static Type getResolvedName(final JavaType target, final JavaType current,
   ...
   *         returns null)
   */
  public static Expression importExpressionIfRequired(final JavaType targetType,
      final List<ImportDeclaration> imports, final Expression value) {
  ...
  public static ReferenceType importParametersForType(final JavaType targetType,
    ...
    final ClassOrInterfaceType cit =
        getClassOrInterfaceType(importTypeIfRequired(targetType, imports, typeToImport));
  ...
  public static NameExpr importTypeIfRequired(final JavaType targetType,
    ...
    if (typeToImport.getEnclosingType() == null) {
      typeToImportExpr =
          new QualifiedNameExpr(new NameExpr(typeToImport.getPackage()
              .getFullyQualifiedPackageName()), typeToImport.getSimpleTypeName());
    } else {
      typeToImportExpr =
          new QualifiedNameExpr(new NameExpr(typeToImport.getEnclosingType()
              .getFullyQualifiedTypeName()), typeToImport.getSimpleTypeName());
    }
   ...
   */
  public static TypeDeclaration locateTypeDeclaration(final CompilationUnit compilationUnit,
