...
public class JavaTypeConverterTest {
  ...
  @Mock
  LastUsed mockLastUsed;
  @Mock
  ProjectOperations mockProjectOperations;
  @Mock
  ...
  public void testConvertFullyQualifiedValueWithOneModulePrefix() {
    ...
    when(mockTypeLocationService.getTopLevelPackageForModule(mockWebPom)).thenReturn(
        topLevelPackage);

    // Invoke
    final JavaType result =
        converter.convertFromText(moduleName + MODULE_PATH_SEPARATOR + topLevelPackage
  ...
  public void testConvertTopLevelPackageWithOneModulePrefix() {
    ...
    // Invoke
    final JavaType result =
        converter.convertFromText(moduleName + MODULE_PATH_SEPARATOR + topLevelPackage, null, null);
  ...
  public void testGetAllPossibleValuesInProjectWhenModulePrefixIsUsed() {
    ...
    when(mockTypeLocationService.getTopLevelPackageForModule(mockOtherModule)).thenReturn(
        topLevelPackage);
    ...
    when(mockTypeLocationService.getTypesForModule(mockOtherModule)).thenReturn(
        Arrays.asList(type1, type2));

    // Invoke
    converter.getAllPossibleValues(mockCompletions, JavaType.class, otherModuleName
        + MODULE_PATH_SEPARATOR + "~.web", OptionContexts.PROJECT, null);

    // Check
    verify(mockCompletions).add(
        new Completion(focusedModuleName + MODULE_PATH_SEPARATOR, AnsiEscapeCode.decorate(
            focusedModuleName + MODULE_PATH_SEPARATOR, AnsiEscapeCode.FG_CYAN), "Modules", 0));
    ...
    verify(mockCompletions).add(
        new Completion(prefix + topLevelPackage, formattedPrefix + topLevelPackage, "", 1));
    verify(mockCompletions).add(
        new Completion(prefix + "~.web.ShouldBeFound", formattedPrefix + "~.web.ShouldBeFound", "",
            1));
  ...
  public void testGetAllPossibleValuesInProjectWhenNoModuleHasFocus() {
    ...
    // Invoke
    converter.getAllPossibleValues(mockCompletions, JavaType.class, "", OptionContexts.PROJECT,
        null);
  ...
  public void testGetAllPossibleValuesInProjectWhenNoModulePrefixIsUsed() {
    ...
    final String topLevelPackage = "com.example";
    when(mockTypeLocationService.getTopLevelPackageForModule(mockFocusedModule)).thenReturn(
        topLevelPackage);
    final String focusedModuleName = "web";
    ...
    when(mockTypeLocationService.getTypesForModule(mockFocusedModule)).thenReturn(
        Arrays.asList(type1, type2));
    ...
    // Check
    verify(mockCompletions).add(
        new Completion(otherModuleName + MODULE_PATH_SEPARATOR, AnsiEscapeCode.decorate(
            otherModuleName + MODULE_PATH_SEPARATOR, AnsiEscapeCode.FG_CYAN), "Modules", 0));
    verify(mockCompletions).add(
        new Completion(topLevelPackage, topLevelPackage, focusedModuleName, 1));
