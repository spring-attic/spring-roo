...
public class PairList<K, V> extends ArrayList<MutablePair<K, V>> {
  ...
  public PairList(final List<? extends K> keys, final List<? extends V> values) {
    ...
    if (keys == null) {
      Validate.isTrue(values == null, "Parameter names must be null if types are null");
    } else {
      Validate.isTrue(values != null, "Parameter names are required if types are provided");
      Validate.isTrue(keys.size() == values.size(), "Expected %d values but found %d", keys.size(),
          values.size());
