...
you use http://spring.io/tools/sts[Spring Tool Suite (STS)], which is our free Eclipse-based IDE
...
Spring Roo Shell includes a default OSGi repository
...
In Spring Roo you simply use the shell to locate new available add-ons on installed 
OSGi repositories. To review the list of known add-ons you can use the 
link:#command-index[addon repository introspect]
or link:#command-index[addon search] command. This lists
all add-ons that are in the installed OSGi repositories mentioned above:
...
top-level package. Let's try this. To view details about
...
To create a patch with Git you can
...
[[roo-marketplace]]
== Publishing - Spring Roo Marketplace
...
The fifth and final part of the reference guide provides appendices and background information that does not neatly belong within the other parts. The information is intended to be treated as a reference and not read consecutively.
