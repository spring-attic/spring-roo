...
+
[IMPORTANT]
In Windows, to force the STS use of JDK VM you need to modify STS params by opening _STS.ini_ and adding the following line *before* the `-vmargs` line:
`-vm [JDK-DIR]/bin/javaw.exe`.
. Open your STS IDE.
...
[lowerroman]
.. Open _"Help >> Install New Software"_.
.. Click _"Available Software sites"_.
.. Click the _"Import" button_.
.. Find the _"$ROO_HOME/conf/sts-sites-bookmarks.xml"_ file and click "OK".
...
.. Click _"Next"_
.. Review the list of software that will be installed. Click _"Next"_ again.
.. Review and accept licence agreement and click _"Finish"_.
. Restart the STS IDE

[#configure-spring-roo]
*Configure Spring Roo 2.0.0*

. Open _"Window > Preferences > Spring > Roo Support"_.
. In _"Roo Support"_ press _"Add"_ new installation button.
. In _"Roo Configure Roo Installation"_ press _"Browse"_ button, then select the the directory in which Spring Roo 2.0.0 was unpacked, `$ROO_HOME`.
+
[align="center"]
...
In this tutorial you will learn to create a complete multimodule web application from scratch using Roo. The application we are going to develop will demonstrate many of the core features offered by Roo.
...
* _Tab completion_: The cornerstone of command-line usability is tab assist. Hit `CTRL+SPACE` (or TAB if you're in a bash-like shell) and Roo will show you the applicable options.
...
. Import existing Spring Roo projects. A simple import of the project using _Eclipse's File > Import > General > Maven Projects_ menu option is sufficient.
. Create new projects, as we will see in the next section.

[[using-spring-roo-build-multimodule-app]]
=== 2. Create projects
...
[[using-spring-roo-project-creation]]
==== Project creation

You can create projects from STS IDE by following the instructions below:

. Open "File > New > Spring Roo Project".
. Select "Project type", for example "Multimodule Standard".
. Select "Roo Installation".
. First press "Next" button and then "Finish" button.
+
[align="center"]
image::sts-new-roo-project.png["New Spring Roo Project", width="50%"]
. Start working on your Roo Shell
+
[align="center"]
...
[[using-spring-roo-northwind-script]]
...
[[using-spring-roo-generating-javadoc]]
