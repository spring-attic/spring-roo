...
All the following parameters are mandatory if `spring.roo.jpa.require.schema-object-name` configuration setting exists and it's value is `true`. See link:#settings-commands[settings commands].
...
+
This parameter is available only when `--versionField` has been specified.
...
+
This parameter is mandatory if `--class` is not specified. Otherwise, using `--class` will cause the parameter `--all` won't be available.
...
+
This parameter is mandatory if `--all` is not specified. Otherwise, using `--all` will cause the parameter `--class` won't be available.

--entity::                   
  Name of the entity which can be used to create the Projection from.
+
This parameter is mandatory if `--class` is specified. Otherwise, not specifying `--class` will cause the parameter `--entity` won't be available.

--fields::
  Comma separated list of entity fields to be included into the Projection.
+
This parameter is mandatory if `--class` is specified. Otherwise, not specifying `--class` will cause the parameter `--fields` won't be available.
...
--column::
  The JPA @Column name.
+
This option is mandatory if `spring.roo.jpa.require.schema-object-name` configuration setting exists and it's `true`. See link:#settings-commands[settings commands].
+
This option is only visible for JPA entities and embeddable classes.
...
--dateFormat::
  Indicates the style of the date format (ignored if dateTimeFormatPattern is specified), adding `style` attribute to `org.springframework.format.annotation.DateTimeFormat` annotation into the field. Possible values are: MEDIUM (style="MS"), NONE (style="-S") and SHORT (style="SS").
+
Default: `MEDIUM`.

--timeFormat::
  Indicates the style of the time format (ignored if dateTimeFormatPattern is specified), adding `style` attribute to `org.springframework.format.annotation.DateTimeFormat` annotation into the field. Possible values are: MEDIUM (style="MS"), NONE (style="-S") and SHORT (style="SS").
+
Default: `NONE`.
...
--type::
  The Java type of the an embeddable class, annotated with `@Embeddable`.
...
[source,sh]
----
roo> field enum --fieldName --type --class --column
----
...
--class::
  ...
  specify the name of the entity in which the new field will be included. If you consider it 
  ...
  entity is in the module that has set the focus. If you consider it necessary, you can also 
...
[source,sh]
----
roo> field list --fieldName --class --type --joinTable --joinColumns --referencedColumns --inverseJoinColumns --inverseReferencedColumns
----
...
[source,sh]
----
roo> field number --fieldName --type --class --column
----
...
[source,sh]
----
roo> field reference --fieldName --type --class --joinColumnName
----
...
[source,sh]
----
roo> field set --fieldName --class --type --joinTable --joinColumns --referencedColumns --inverseJoinColumns --inverseReferencedColumns
----
...
[source,sh]
----
roo> field string --fieldName --class --column
----
...
[source,sh]
----
roo> jpa setup --provider --database --module
----
