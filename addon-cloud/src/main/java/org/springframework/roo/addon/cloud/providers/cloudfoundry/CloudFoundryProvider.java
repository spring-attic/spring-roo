...
public class CloudFoundryProvider implements CloudProvider {
  ...
  public static final String DESCRIPTION =
      "Setup Cloud Foundry Maven Plugin on your Spring Roo Application";

  private static final Logger LOGGER = Logger.getLogger(CloudFoundryProvider.class.getName());
  ...
  public static void updatePlugins(String pluginConfiguration, ProjectOperations projectOperations) {
    ...
    // Generate Plugin
    Plugin cloudFoundryMvnPlugin =
        new Plugin("org.cloudfoundry", "cf-maven-plugin", "1.0.4", conf, null, null);

    // Adding plugin
    projectOperations.addBuildPlugin(projectOperations.getFocusedModuleName(),
  ...
  public static void showInfo(String configuration) {
    ...
    if (StringUtils.isNotBlank(configuration)) {
      ...
      for (String configurationTag : configurationTags) {
        ...
        if (keyValue.length == 2) {
          LOGGER.log(Level.INFO,
              String.format("   <%s>%s</%s>", keyValue[0], keyValue[1], keyValue[0]));
        }
      }
      LOGGER.log(Level.INFO, "</configuration>");
    } else {
      LOGGER.log(Level.INFO, "WARNING: You don't specify any configuration.");
    }
    ...
    LOGGER.log(Level.INFO, "cf:app 	          			Show details of an application.");
    LOGGER.log(Level.INFO, "cf:delete 	      			Delete an application.");
    LOGGER.log(Level.INFO, "cf:env 	          			Show an application's environment variables.");
    LOGGER.log(Level.INFO, "cf:help 	      			Show documentation for all available commands.");
    LOGGER.log(Level.INFO, "cf:push 	      			Push and optionally start an application.");
    LOGGER.log(Level.INFO,
    ...
    LOGGER.log(Level.INFO, "cf:logs 	                Tail application logs.");
    LOGGER.log(Level.INFO, "cf:recentLogs               Show recent application logs.");
    LOGGER.log(Level.INFO, "cf:scale 	                Scale the application instances up or down.");
    LOGGER.log(Level.INFO, "cf:services                 Show a list of provisioned services.");
    LOGGER.log(Level.INFO, "cf:service-plans 	        Show a list of available service plans.");
    LOGGER.log(Level.INFO, "cf:create-services 	        Create services defined in the pom.");
    LOGGER.log(Level.INFO, "cf:delete-services 	        Delete services defined in the pom.");
    LOGGER.log(Level.INFO, "cf:bind-services 	        Bind services to an application.");
    LOGGER.log(Level.INFO, "cf:unbind-services 	        Unbind services from an application.");
    LOGGER.log(Level.INFO,
