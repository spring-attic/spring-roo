...
public interface MemberDetailsDecorator {
	 ...
	 * new instance is only returned if an actual change to the result is required.</em> An implementation can
	 * safely return the same {@link MemberDetails} as it was passed if in doubt, as it will always be re-invoked
	 * later on if another decorator changes the {@link MemberDetails} instance. Thus even decorators that depend
