...
public class MethodParameter extends Pair<JavaType, JavaSymbolName> {
	 ...
	 * @return
	 */
	public static List<MethodParameter> asList(final List<Pair<JavaType, JavaSymbolName>> parameters) {
		final List<MethodParameter> list = new ArrayList<MethodParameter>();
		if (parameters != null) {
			for (final Pair<JavaType, JavaSymbolName> parameter : parameters) {
				list.add(new MethodParameter(parameter.getKey(), parameter.getValue()));
			}
		}
		return list;
	}
