...
public class JavaPackageConverterTest {
	 ...
	 */
	private void assertConvertFromValidText(final String text, final String optionContext, final String expectedPackage) {
		// Set up
	...
	public void testConvertFromSimplePackageNameInNoContext() {
		assertEquals("foo", converter.convertFromText("FOO", JavaPackage.class, null).getFullyQualifiedPackageName());
	...
	private Pom setUpMockPom(final String path, final String... types) {
		...
		when(mockTypeLocationService.getTypesForModule(path)).thenReturn(Arrays.asList(types));
		return mockPom;
	}
}
