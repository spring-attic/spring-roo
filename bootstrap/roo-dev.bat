...
for /f "usebackq tokens=* delims= " %%a in (%ROO_CLASSPATH_FILE%) do (
	set ROO_CP=%%a
)

if "%ROO_ADDON_CLASSPATH_FILE%" == "" goto run
for /f "usebackq tokens=* delims= " %%a in (%ROO_ADDON_CLASSPATH_FILE%) do (
	set EXTENDED_CP=%%a
)
...
:launch
java -Djline.nobell=true -DdevelopmentMode=true %ROO_OPTS% -Droo.home="%USERPROFILE%\roo-dev" -cp %ROO_CP% -Djava.ext.dirs="%USERPROFILE%\roo-dev\work;%JAVA_HOME%\jre\lib\ext" org.springframework.roo.bootstrap.Bootstrap "classpath:/roo-bootstrap.xml" %*
