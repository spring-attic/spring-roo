...
public class AutoProcessor {
	...
	private static void processAutoDeploy(Map configMap, BundleContext context) {
		...
		while (st.hasMoreTokens()) {
			...
			if (s.equals(AUTO_DEPLOY_INSTALL_VALUE) || s.equals(AUTO_DEPLOY_START_VALUE) || s.equals(AUTO_DEPLOY_UPDATE_VALUE) || s.equals(AUTO_DEPLOY_UNINSTALL_VALUE)) {
				actionList.add(s);
			}
		}
	...
	private static void processAutoProperties(Map configMap, BundleContext context) {
		...
		StartLevel sl = (StartLevel) context.getService(context.getServiceReference(org.osgi.service.startlevel.StartLevel.class.getName()));
	...
	private static String nextLocation(StringTokenizer st) {
		...
		if (st.countTokens() > 0) {
			...
			// Handle case where end of token stream and still got data
			if ((!exit) && (tokStarted)) {
				retVal = tokBuf.toString();
			}
		}
