...
public class PermissionEvaluatorMetadata extends AbstractItdTypeDetailsProvidingMetadataItem
{
	...
	protected PermissionEvaluatorMetadata(String identifier, JavaType aspectName, PhysicalTypeMetadata governorPhysicalTypeMetadata, final MemberDetails governorDetails)
	{
		...
		if (!governorDetails.isMethodDeclaredByAnother(methodName, hasPermissionParameterTypes, getId()))
		{
			...
			MethodMetadataBuilder hasPermissionMethodMetadataBuilder = new MethodMetadataBuilder(getId(), PUBLIC, methodName, JavaType.BOOLEAN_PRIMITIVE,
					AnnotatedJavaType.convertFromJavaTypes(hasPermissionParameterTypes), hasPermissionParameterNames, hasPermissionBodyBuilder);
		...
		if (!governorDetails.isMethodDeclaredByAnother(methodName, hasPermissionParameterTypes2, getId()))
		{
			...
			MethodMetadataBuilder hasPermissionMethodMetadataBuilder2 = new MethodMetadataBuilder(getId(), PUBLIC, new JavaSymbolName("hasPermission"), JavaType.BOOLEAN_PRIMITIVE,
					AnnotatedJavaType.convertFromJavaTypes(hasPermissionParameterTypes2), hasPermissionParameterNames2, hasPermissionBodyBuilder2);
