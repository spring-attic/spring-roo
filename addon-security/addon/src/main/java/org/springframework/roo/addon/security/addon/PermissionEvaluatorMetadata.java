...
public class PermissionEvaluatorMetadata extends AbstractItdTypeDetailsProvidingMetadataItem {
  private static final String PROVIDES_TYPE_STRING = PermissionEvaluatorMetadata.class.getName();
  ...
  protected PermissionEvaluatorMetadata(String identifier, final JavaType aspectName,
      final PhysicalTypeMetadata governorPhysicalTypeMetadata, final MemberDetails governorDetails,
    ...
    if (!governorDetails.isMethodDeclaredByAnother(hasPermissionMethodName,
      ...
      hasPermissionBodyBuilder
          .append("\n\t\treturn checkManagedPermissions(authentication, targetObject, permission);");

      MethodMetadataBuilder hasPermissionMethodMetadataBuilder =
          new MethodMetadataBuilder(getId(), PUBLIC, hasPermissionMethodName,
              JavaType.BOOLEAN_PRIMITIVE,
              AnnotatedJavaType.convertFromJavaTypes(hasPermissionParameterTypes),
    ...
    if (!governorDetails.isMethodDeclaredByAnother(hasPermissionMethodName,
      ...
      hasPermissionParameterNames2.add(new JavaSymbolName("authentication"));
      ...
      MethodMetadataBuilder hasPermissionMethodMetadataBuilder2 =
          new MethodMetadataBuilder(getId(), PUBLIC, new JavaSymbolName("hasPermission"),
              JavaType.BOOLEAN_PRIMITIVE,
              AnnotatedJavaType.convertFromJavaTypes(hasPermissionParameterTypes),
    ...
    JavaSymbolName checkManagedPermissionsMethodName =
        new JavaSymbolName("checkManagedPermissions");
    ...
    if (!governorDetails.isMethodDeclaredByAnother(checkManagedPermissionsMethodName,
      ...
      checkManagedPermissionsParameterNames.add(new JavaSymbolName("authentication"));
      ...
      final InvocableMemberBodyBuilder checkManagedPermissionsBodyBuilder =
      ...
      for (Entry<JavaType, String> entrySet : domainTypeToPlurals.entrySet()) {
        for (Permission permission : Permission.values()) {
          ...
          checkManagedPermissionsBodyBuilder.append(String.format(
              "\n\t\t\treturn %s(authentication, (%s)targetObject);", permissionName, entrySet
                  .getKey().getFullyQualifiedTypeName()));
      ...
      MethodMetadataBuilder hasPermissionMethodMetadataBuilder =
          new MethodMetadataBuilder(getId(), PUBLIC, checkManagedPermissionsMethodName,
              JavaType.BOOLEAN_PRIMITIVE,
              AnnotatedJavaType.convertFromJavaTypes(checkManagedPermissionsParameterTypes),
    ...
    for (Entry<JavaType, String> entrySet : domainTypeToPlurals.entrySet()) {
      for (Permission permission : Permission.values()) {
        ...
        if (!governorDetails.isMethodDeclaredByAnother(isAllowedMethodName,
            isAllowedParameterTypes, getId())) {
          ...
          MethodMetadataBuilder isAllowedMethodMetadataBuilder =
              new MethodMetadataBuilder(getId(), PUBLIC, isAllowedMethodName,
                  JavaType.BOOLEAN_PRIMITIVE,
                  AnnotatedJavaType.convertFromJavaTypes(isAllowedParameterTypes),
