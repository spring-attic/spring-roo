...
/*        getPropFileOperations().addProperties(Path.SRC_MAIN_RESOURCES
	...
	public void configureTemplateMessage(String from, String subject) {
		...
		JavaType target = new JavaType(getProjectOperations().getFocusedTopLevelPackage().getFullyQualifiedPackageName().concat(".config.SimpleMailMessageConfig"));
		...
		File targetFile = new File(getTypeLocationService().getPhysicalTypeCanonicalPath(declaredByMetadataId));
		...
		if(!targetFile.exists()){
			...
			cidBuilder.addAnnotation(new AnnotationMetadataBuilder(new JavaType("org.springframework.context.annotation.Configuration")));

			// Including @RooSimpleMailMessageConfig 
			cidBuilder.addAnnotation(new AnnotationMetadataBuilder(new JavaType(RooSimpleMailMessageConfig.class)));
...
/*        getPropFileOperations().addProperties(Path.SRC_MAIN_RESOURCES
	...
	public void injectEmailTemplate(final JavaType targetType,
        ...
        final ClassOrInterfaceTypeDetails existing = getTypeLocationService()
