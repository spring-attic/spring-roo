	...
	private MemberDetails findMemberDetails(JavaType type) {
		for (ClassOrInterfaceTypeDetails coitd : typeLocationService.findClassesOrInterfaceDetailsWithAnnotation(ANNOTATION_TYPE)) {
			...
			if (foundTypes.isEmpty()) {
				...
				return memberDetailsScanner.getMemberDetails(ServiceLayerProvider.class.getName(), foundTypes.get(0));
			}
			throw new IllegalStateException("Detected " + foundTypes.size() + " project types annotated with @" + RooService.class.getSimpleName() + " that defines " + type.getSimpleTypeName() + " as domain type.");
		}
		return null;
	}
