...
public class ServiceClassMetadata extends AbstractItdTypeDetailsProvidingMetadataItem
{
	 ...
	 * @param governorDetails
	 *            (required)
	 * @param annotationValues
	 *            (required)
	 * @param domainTypeToIdTypeMap
	 *            (required)
	 * @param allCrudAdditions
	 *            any additions to be made to the service class in order to
	 *            invoke lower-layer methods (required)
	 * @param domainTypePlurals
	 *            the plurals of each domain type managed by the service
	 */
	public ServiceClassMetadata(final String identifier, final JavaType aspectName, final PhysicalTypeMetadata governorPhysicalTypeMetadata, final MemberDetails governorDetails,
			final ServiceAnnotationValues annotationValues, final Map<JavaType, JavaType> domainTypeToIdTypeMap, final Map<JavaType, Map<ServiceLayerMethod, MemberTypeAdditions>> allCrudAdditions,
			final Map<JavaType, String> domainTypePlurals, String serviceName)
	{
		...
		for (final Entry<JavaType, JavaType> entry : domainTypeToIdTypeMap.entrySet())
		{
			...
			for (final ServiceLayerMethod method : ServiceLayerMethod.values())
			{
				...
				final String permissionName = method.getPermissionName(annotationValues, domainType, domainTypePlurals.get(domainType));
				if (methodName != null && !governorDetails.isMethodDeclaredByAnother(methodName, method.getParameterTypes(domainType, idType), getId()))
				{
					...
					List<JavaSymbolName> parameterNames = method.getParameterNames(domainType, idType);
					MethodMetadataBuilder methodMetadataBuilder = new MethodMetadataBuilder(getId(), Modifier.PUBLIC, methodName, method.getReturnType(domainType),
							AnnotatedJavaType.convertFromJavaTypes(method.getParameterTypes(domainType, idType)), parameterNames, bodyBuilder);
					...
					boolean requireAuthentication = annotationValues.requireAuthentication();

					if (method.getKey().equals(CustomDataKeys.PERSIST_METHOD.name()) || method.getKey().equals(CustomDataKeys.MERGE_METHOD.name()))
					{
						isCreateOrUpdateMethod = true;
					}

					if (method.getKey().equals(CustomDataKeys.REMOVE_METHOD.name()))
					{
						isDeleteMethod = true;
					}

					if (method.getKey().equals(CustomDataKeys.FIND_ALL_METHOD.name()) || method.getKey().equals(CustomDataKeys.FIND_ENTRIES_METHOD.name())
							|| method.getKey().equals(CustomDataKeys.FIND_METHOD.name()) || method.getKey().equals(CustomDataKeys.COUNT_ALL_METHOD))
					{
						isReadMethod = true;
					}

					if (method.getKey().equals(CustomDataKeys.PERSIST_METHOD.name()) || method.getKey().equals(CustomDataKeys.MERGE_METHOD.name())
							|| method.getKey().equals(CustomDataKeys.REMOVE_METHOD.name()) || method.getKey().equals(CustomDataKeys.FIND_METHOD.name()))
					{
						usesDomainTypeMethod = true;
					}

					if (annotationValues.getAuthorizedCreateOrUpdateRoles().length > 0 && isCreateOrUpdateMethod)
					{
						requireAuthentication = true;
						addRoles(preAuthorizeValue, annotationValues.getAuthorizedCreateOrUpdateRoles());
					}

					if (annotationValues.getAuthorizedReadRoles().length > 0 && isReadMethod)
					{
						requireAuthentication = true;
						addRoles(preAuthorizeValue, annotationValues.getAuthorizedReadRoles());
					}

					if (annotationValues.getAuthorizedDeleteRoles().length > 0 && isDeleteMethod)
					{
						requireAuthentication = true;
						addRoles(preAuthorizeValue, annotationValues.getAuthorizedDeleteRoles());
					}

					if (annotationValues.usePermissionEvaluator() && usesDomainTypeMethod)
					{
						requireAuthentication = true;
						preAuthorizeValue.append(" OR hasPermission(" + (parameterNames.size() == 0 ? "#" : "#" + parameterNames.get(0).getSymbolName()) + ", '" + serviceName + ":" + permissionName
								+ "'" + ")");
					}
					...
					{
						final AnnotationMetadataBuilder annotationMetadataBuilder = new AnnotationMetadataBuilder(PRE_AUTHORIZE);
						annotationMetadataBuilder.addStringAttribute("value", preAuthorizeValue.toString());
						methodMetadataBuilder.addAnnotation(annotationMetadataBuilder.build());
					}
