...
public class EntityLayerProvider extends CoreLayerProvider {
	...
	public MemberTypeAdditions getMemberTypeAdditions(final String callerMID, final String methodIdentifier, final JavaType targetEntity, final JavaType idType, final MethodParameter... callerParameters) {
		...
		// Get the CRUD-related values of this entity's @RooJpaActiveRecord annotation
		final JpaCrudAnnotationValues annotationValues = jpaActiveRecordMetadataProvider.getAnnotationValues(targetEntity);
	...
	private String getPlural(final JavaType javaType) {
		final String key = PluralMetadata.createIdentifier(javaType, typeLocationService.getTypePath(javaType));
	 ...
	 */
	void setJpaActiveRecordMetadataProvider(final JpaActiveRecordMetadataProvider jpaActiveRecordMetadataProvider) {
