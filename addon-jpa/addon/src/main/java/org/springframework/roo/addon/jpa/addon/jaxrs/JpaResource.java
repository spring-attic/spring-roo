...
public class JpaResource implements ResourceMarker {
    ...
    public JsonObject getPersistenceInfo() {
        ...
        String persistenceFile = getPathResolver().getFocusedIdentifier(
                org.springframework.roo.project.Path.SRC_MAIN_RESOURCES,
                "META-INF/persistence.xml");

        if (getFileManager().exists(persistenceFile)) {

            // Reading persistence.xml
            final InputStream inputStream = getFileManager().getInputStream(
                    persistenceFile);
                    ...
                    if (properties.item(i).hasAttributes()) {
                        ...
                        if (nameAttr.equals("datanucleus.ConnectionURL")) {
                            databaseUrl = valueAttr;
                                ...
                                .equals("datanucleus.ConnectionUserName")) {
                            username = valueAttr;
                        }
                        else if (nameAttr
                                .equals("datanucleus.ConnectionPassword")) {
                            password = valueAttr;
                        }
                    }
                }
                ...
                for (int i = 0; i < properties.getLength(); i++) {
                    if (properties.item(i).hasAttributes()) {
                        ...
                        if (nameAttr.equals("datanucleus.ConnectionDriverName")) {
                            database = getDatabaseByDriverClassName(valueAttr,
                                    databaseUrl);
                        }
                    }
                }
            ...
            if (!persistenceProvider.equals("DATANUCLEUS")) {
                ...
                for (String databaseProperty : databaseProperties) {
                    ...
                    if (key.equals("database.driverClassName")) {
                        database = getDatabaseByDriverClassName(value,
                                databaseUrl);
                    }
        ...
        // Returning JSON
        JsonObjectBuilder jsonBuilder = Json.createObjectBuilder();
        jsonBuilder.add("persistenceProvider", persistenceProvider)
                .add("database", database).add("databaseUrl", databaseUrl)
    ...
    public JsonObject setupJpa(@FormParam("providerName") String provider,
        ...
        String persistenceCommand = "jpa setup --provider \"" + provider
                + "\" --database " + database;

        if (username != null || !"".equals(username)) {
            persistenceCommand = persistenceCommand.concat(" --userName ")
                    .concat(username);
        }

        if (password != null || !"".equals(password)) {
            persistenceCommand = persistenceCommand.concat(" --password ")
                    .concat(password);
        }

        boolean status = getShell().executeCommand(persistenceCommand);

        String message = "Persistence was configured to use '" + provider
                + "' and '" + database + "'!";
        if (!status) {
     ...
     * @return
     */
    private String getDatabaseByDriverClassName(String driverClassName,
            String databaseUrl) {
        ...
        if (jpaOperations == null) {
            ...
            try {
                ...
                for (ServiceReference<?> ref : references) {
                    jpaOperations = (JpaOperations) this.context
                            .getService(ref);
