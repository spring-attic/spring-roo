...
public class DynamicFinderServicesImpl implements DynamicFinderServices {
  ...
  private Set<JavaSymbolName> createFinders(final FieldMetadata field,
      final Set<JavaSymbolName> finders, final String prepend, final boolean isFirst) {
  ...
  public List<JavaSymbolName> getFinders(final MemberDetails memberDetails, final String plural,
      final int depth, final Set<JavaSymbolName> exclusions) {
    Validate.notNull(memberDetails, "Member details required");
  ...
  private Token getFirstToken(final SortedSet<FieldToken> fieldTokens, final String finder,
      final String originalFinder, final String simpleTypeName) {
    ...
    if (finder.length() > 0) {
      ...
      throw new FinderFieldTokenMissingException("Dynamic finder is unable to match '" + finder
          + "' token of '" + originalFinder + "' finder definition in " + simpleTypeName + ".java");
    }

    return null; // Finder does not start with reserved or field token
  }

  private String getJpaQuery(final List<Token> tokens, final String simpleTypeName,
      final JavaSymbolName finderName, final String plural, final String entityName) {
    final String typeName = StringUtils.defaultIfEmpty(entityName, simpleTypeName);
    ...
    for (final Token token : tokens) {
      if (token instanceof ReservedToken) {
        ...
        final String fieldName = lastFieldToken.getField().getFieldName().getSymbolName();
  ...
  private String getJpaCountQuery(final List<Token> tokens, final String simpleTypeName,
      final JavaSymbolName finderName, final String plural, final String entityName) {
  ...
  private List<JavaSymbolName> getParameterNames(final List<Token> tokens,
    ...
    for (int i = 0; i < tokens.size(); i++) {
      ...
      if (token instanceof FieldToken) {
        final String fieldName = ((FieldToken) token).getField().getFieldName().getSymbolName();
        ...
        if ("Between".equals(token.getValue())) {
          ...
          if (field instanceof FieldToken) {
            final JavaSymbolName fieldName = parameterNames.get(parameterNames.size() - 1);
            // Remove the last field token
            parameterNames.remove(parameterNames.size() - 1);

            // Replace by a min and a max value
            parameterNames.add(new JavaSymbolName("min"
                + fieldName.getSymbolNameCapitalisedFirstLetter()));
  ...
  public QueryHolder getQueryHolder(final MemberDetails memberDetails,
    ...
    final String jpaQuery = getJpaQuery(tokens, simpleTypeName, finderName, plural, entityName);
    final List<JavaType> parameterTypes = getParameterTypes(tokens, finderName, plural);
    final List<JavaSymbolName> parameterNames = getParameterNames(tokens, finderName, plural);
  ...
  public QueryHolder getCountQueryHolder(final MemberDetails memberDetails,
    ...
    final String jpaQuery =
        getJpaCountQuery(tokens, simpleTypeName, finderName, plural, entityName);
    final List<JavaType> parameterTypes = getParameterTypes(tokens, finderName, plural);
    final List<JavaSymbolName> parameterNames = getParameterNames(tokens, finderName, plural);
    return new QueryHolder(jpaQuery, parameterTypes, parameterNames, tokens);
  }
  ...
  private boolean isNumberOrDate(final JavaType fieldType) {
    return fieldType.equals(JavaType.DOUBLE_OBJECT) || fieldType.equals(JavaType.FLOAT_OBJECT)
        || fieldType.equals(JavaType.INT_OBJECT) || fieldType.equals(LONG_OBJECT)
  ...
  private Set<JavaSymbolName> populateFinders(final Set<JavaSymbolName> finders,
      final FieldMetadata field, final String prepend, final boolean isFirst, final String keyWord) {
    final Set<JavaSymbolName> tempFinders = new HashSet<JavaSymbolName>();
    ...
    if (isTransient(field)) {
      ...
      for (final JavaSymbolName finder : finders) {
        ...
        if (!finderName.contains(field.getFieldName().getSymbolNameCapitalisedFirstLetter())) {
          tempFinders.add(new JavaSymbolName(finderName + prepend
              + field.getFieldName().getSymbolNameCapitalisedFirstLetter() + keyWord));
        }
      }
    }
  ...
  private List<Token> tokenize(final MemberDetails memberDetails, final JavaSymbolName finderName,
    ...
    if (finder.contains(findBy)) {
      throw new InvalidFinderException("Dynamic finder definition for '"
          + finderName.getSymbolName() + "' in " + simpleTypeName + ".java is invalid");
    }
    ...
    for (final MethodMetadata method : getLocatedMutators(memberDetails)) {
      final FieldMetadata field =
          BeanInfoUtils.getFieldForPropertyName(memberDetails, method.getParameterNames().get(0));
    ...
    while (finder.length() > 0) {
      final Token token =
          getFirstToken(fieldTokens, finder, finderName.getSymbolName(), simpleTypeName);
      if (token != null) {
