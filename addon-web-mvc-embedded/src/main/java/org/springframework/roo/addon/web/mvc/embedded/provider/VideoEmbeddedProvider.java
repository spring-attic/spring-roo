...
public class VideoEmbeddedProvider extends AbstractEmbeddedProvider {
    ...
    public boolean install(final String viewName,
        ...
        installTagx("video");
        Element video = new XmlElementBuilder("embed:video", XmlUtils
    ...
    private String getViddlerId(final String url) {
        String xml = sendHttpGetRequest("http://lab.viddler.com/services/oembed/?url="
                + url + "&type=simple&format=xml");
        if (xml != null) {
            try {
                Document doc = XmlUtils.readXml(new ByteArrayInputStream(xml
                        .getBytes()));
                Element movie = XmlUtils.findRequiredElement(
    ...
    public enum VideoProvider implements EmbeddedCompletor {
        YOUTUBE, GOOGLE_VIDEO, VIMEO, VIDDLER, SCREENR;
