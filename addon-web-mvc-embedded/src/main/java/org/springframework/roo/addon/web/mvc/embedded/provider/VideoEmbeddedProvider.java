...
public class VideoEmbeddedProvider extends AbstractEmbeddedProvider {
  ...
  private String getViddlerId(final String url) {
    ...
    if (xml != null) {
      try {
        final Document doc = XmlUtils.readXml(new ByteArrayInputStream(xml.getBytes()));
        final Element movie =
            XmlUtils.findRequiredElement("//param[@name='movie']", doc.getDocumentElement());
  ...
  public boolean install(final String viewName, final Map<String, String> options) {
    ...
    installTagx("video");
    final Element video =
        new XmlElementBuilder("embed:video", XmlUtils.getDocumentBuilder().newDocument())
