...
public abstract class DbreTypeUtils {
  ...
  private static final Map<JavaType, JavaSymbolName> SCHEMA_ATTRIBUTES =
      new LinkedHashMap<JavaType, JavaSymbolName>();
  ...
  // Linked to preserve the iteration order below
  private static final Map<JavaType, JavaSymbolName> TABLE_ATTRIBUTES =
      new LinkedHashMap<JavaType, JavaSymbolName>();
  ...
  public static JavaType findTypeForTable(
      final Iterable<ClassOrInterfaceTypeDetails> managedEntities, final Table table) {
  ...
  public static JavaType findTypeForTableName(
      final Iterable<ClassOrInterfaceTypeDetails> managedEntities, final String tableName,
    ...
    for (final ClassOrInterfaceTypeDetails managedEntity : managedEntities) {
      ...
      if (tableName.equals(getTableName(managedEntity))
          && (!DbreModelService.NO_SCHEMA_REQUIRED.equals(managedSchemaName) || schemaName
  ...
  private static <T> T getAnnotationAttribute(final MemberHoldingTypeDetails type,
      final JavaType annotationType, final JavaSymbolName attributeName) {
  ...
  private static String getFirstNonBlankAttributeValue(
    ...
    for (final Entry<JavaType, JavaSymbolName> entry : annotationAttributes.entrySet()) {
      final String attributeValue =
          getAnnotationAttribute(annotatedType, entry.getKey(), entry.getValue());
  ...
  public static String getSchemaName(final MemberHoldingTypeDetails entityDetails) {
    Validate.notNull(entityDetails, "MemberHoldingTypeDetails type required");
  ...
  public static JavaType suggestTypeNameForNewTable(final String tableName,
    ...
    final StringBuilder result = new StringBuilder();
    if (javaPackage != null && StringUtils.isNotBlank(javaPackage.getFullyQualifiedPackageName())) {
