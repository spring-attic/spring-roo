...
public class ConnectionProviderImpl implements ConnectionProvider {
  ...
  public Connection getConnection(final Map<String, String> map, final boolean displayAddOns)
      throws RuntimeException {
    return getConnection(getProps(map), displayAddOns);
  }

  public Connection getConnection(final Properties props, final boolean displayAddOns)
      throws RuntimeException {
    Validate.notEmpty(props, "Connection properties must not be null or empty");
    ...
    final String driverClassName = props.getProperty("database.driverClassName");
    final Driver driver = jdbcDriverManager.loadDriver(driverClassName, displayAddOns);
    Validate.notNull(driver, "JDBC driver not available for '%s'", driverClassName);
    try {
  ...
  public Connection getConnectionViaJndiDataSource(final String jndiDataSource,
      final Map<String, String> map, final boolean displayAddOns) throws RuntimeException {
    try {
      final InitialContext context = new InitialContext(getProps(map));
      final DataSource dataSource = (DataSource) context.lookup(jndiDataSource);
