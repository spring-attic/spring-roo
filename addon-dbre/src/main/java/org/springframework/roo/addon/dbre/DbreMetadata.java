...
public class DbreMetadata extends AbstractItdTypeDetailsProvidingMetadataItem {
    ...
    private void addManyToManyFields(final Table table) {
        ...
        for (Table joinTable : database.getTables()) {
            ...
            String errMsg = "table in join table '"
                    + joinTable.getName()
            ...
            ForeignKey foreignKey2 = iter.next(); // Second and last foreign key
                                                  // in set
            ...
            Integer tableCount = owningSideTables.containsKey(owningSideTable) ? owningSideTables
                    .get(owningSideTable) + 1 : 0;
            owningSideTables.put(owningSideTable, tableCount);
            String fieldSuffix = owningSideTables.get(owningSideTable) > 0 ? String
            ...
            if (owningSideTable.equals(table)) {
                ...
                FieldMetadataBuilder fieldBuilder = getManyToManyOwningSideField(
            ...
            if (inverseSideTable.equals(table)) {
                ...
                JavaSymbolName mappedByFieldName = new JavaSymbolName(
                        getInflectorPlural(DbreTypeUtils
                                .suggestFieldName(inverseSideTable))
                                + (sameTable ? "1" : fieldSuffix));
                FieldMetadataBuilder fieldBuilder = getManyToManyInverseSideField(
    ...
    private void addOneToOneFields(final Table table) {
        ...
        Map<JavaSymbolName, FieldMetadataBuilder> uniqueFields = new LinkedHashMap<JavaSymbolName, FieldMetadataBuilder>();
        ...
        for (ForeignKey foreignKey : table.getImportedKeys()) {
            ...
            String foreignSchemaName = importedKeyForeignTable.getSchema()
                    .getName();
            Short keySequence = foreignKey.getKeySequence();
            String fieldSuffix = keySequence != null && keySequence > 0 ? String
                    .valueOf(keySequence) : "";
            JavaSymbolName fieldName = new JavaSymbolName(
                    DbreTypeUtils.suggestFieldName(foreignTableName)
                            + fieldSuffix);
            JavaType fieldType = DbreTypeUtils.findTypeForTableName(
            ...
            FieldMetadataBuilder fieldBuilder = getOneToOneOrManyToOneField(
                    fieldName, fieldType, foreignKey, ONE_TO_ONE, false);
            uniqueFields.put(fieldName, fieldBuilder);
        }

        for (FieldMetadataBuilder fieldBuilder : uniqueFields.values()) {
            addToBuilder(fieldBuilder);
        ...
        for (ForeignKey exportedKey : table.getExportedKeys()) {
            ...
            String foreignSchemaName = exportedKeyForeignTable.getSchema()
                    .getName();
            ...
            FieldMetadataBuilder fieldBuilder = getOneToOneMappedByField(
    ...
    private void addOneToManyFields(final Table table) {
        ...
        for (ForeignKey exportedKey : table.getExportedKeys()) {
            ...
            if (exportedKey.getReferenceCount() == 1) {
                Reference reference = exportedKey.getReferences().iterator()
                        .next();
            ...
            FieldMetadataBuilder fieldBuilder = getOneToManyMappedByField(
    ...
    private void addManyToOneFields(final Table table) {
        ...
        for (ForeignKey foreignKey : table.getImportedKeys()) {
            ...
            if (foreignKey.getReferenceCount() == 1) {
                Reference reference = foreignKey.getReferences().iterator()
                        .next();
    ...
    private FieldMetadataBuilder getManyToManyOwningSideField(
        ...
        JavaType element = DbreTypeUtils.findTypeForTable(managedEntities,
                inverseSideTable);
        Assert.notNull(
                element,
        ...
        JavaType fieldType = new JavaType(SET.getFullyQualifiedTypeName(), 0,
        ...
        AnnotationMetadataBuilder manyToManyBuilder = new AnnotationMetadataBuilder(
                MANY_TO_MANY);
        annotations.add(manyToManyBuilder);

        // Add @JoinTable annotation
        AnnotationMetadataBuilder joinTableBuilder = new AnnotationMetadataBuilder(
                JOIN_TABLE);
        ...
        for (Reference reference : firstKeyReferences) {
            AnnotationMetadataBuilder joinColumnBuilder = getJoinColumnAnnotation(
    ...
    private FieldMetadataBuilder getOneToOneMappedByField(
        ...
        List<AnnotationMetadataBuilder> annotations = new ArrayList<AnnotationMetadataBuilder>();
        AnnotationMetadataBuilder oneToOneBuilder = new AnnotationMetadataBuilder(
                ONE_TO_ONE);
    ...
    private void excludeFieldsInToStringAnnotation(final String fieldName) {
        AnnotationMetadata toStringAnnotation = governorTypeDetails
        ...
        AnnotationMetadataBuilder toStringAnnotationBuilder = new AnnotationMetadataBuilder(
    ...
    private FieldMetadataBuilder getOneToManyMappedByField(
            ...
            final CascadeAction onUpdate, final CascadeAction onDelete) {
        JavaType element = DbreTypeUtils.findTypeForTableName(managedEntities,
                foreignTableName, foreignSchemaName);
        ...
        JavaType fieldType = new JavaType(SET.getFullyQualifiedTypeName(), 0,
                DataType.TYPE, null, Arrays.asList(element));
        // Add @OneToMany annotation
        List<AnnotationMetadataBuilder> annotations = new ArrayList<AnnotationMetadataBuilder>();
        AnnotationMetadataBuilder oneToManyBuilder = new AnnotationMetadataBuilder(
                ONE_TO_MANY);
    ...
    private AnnotationMetadataBuilder getJoinColumnAnnotation(
        ...
        AnnotationMetadataBuilder joinColumnBuilder = new AnnotationMetadataBuilder(
                JOIN_COLUMN);
        joinColumnBuilder
    ...
    private AnnotationMetadataBuilder getJoinColumnsAnnotation(
        ...
        for (Reference reference : references) {
            AnnotationMetadataBuilder joinColumnAnnotation = getJoinColumnAnnotation(
    ...
    private void addOtherFields(final Table table) {
        ...
        for (Column column : table.getColumns()) {
            ...
            boolean hasEmbeddedIdField = isEmbeddedIdField(fieldName)
                    && !isCompositeKeyField;
            ...
            FieldMetadataBuilder fieldBuilder = getField(fieldName, column,
    ...
    private boolean isCompositeKeyField(final JavaSymbolName fieldName) {
        ...
        for (FieldMetadata field : identifierHolder
    ...
    private boolean isCompositeKeyColumn(final Column column) {
        ...
        for (FieldMetadata field : identifierHolder
            ...
            for (AnnotationMetadata annotation : field.getAnnotations()) {
                ...
                AnnotationAttributeValue<?> nameAttribute = annotation
    ...
    private boolean hasField(final JavaSymbolName fieldName,
        ...
        for (FieldMetadata governorField : governorFields) {
            governorFieldAnnotations: for (AnnotationMetadata governorFieldAnnotation : governorField
                    .getAnnotations()) {
                if (governorFieldAnnotation.getAnnotationType().equals(COLUMN)
                        || governorFieldAnnotation.getAnnotationType().equals(
                                JOIN_COLUMN)) {
                    AnnotationAttributeValue<?> name = governorFieldAnnotation
                            .getAttribute(new JavaSymbolName(NAME));
                    if (name == null) {
                        continue governorFieldAnnotations;
                    }
                    for (AnnotationMetadata annotationMetadata : annotations) {
                        AnnotationAttributeValue<?> columnName = annotationMetadata
                                .getAttribute(new JavaSymbolName(NAME));
