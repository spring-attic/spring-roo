...
public class RepositoryJpaOperationsImpl implements RepositoryJpaOperations {
  ...
  private JavaType addRepositoryCustom(JavaType domainType, JavaType repositoryType,
    ...
    JavaType implType = new JavaType(interfaceType.getFullyQualifiedTypeName().concat("Impl"));
    ...
    final String implMId =
        PhysicalTypeIdentifier
            .createIdentifier(implType, getPathResolver().getPath(implIdentifier));
    final ClassOrInterfaceTypeDetailsBuilder implBuilder =
        new ClassOrInterfaceTypeDetailsBuilder(implMId, Modifier.PUBLIC, implType,
            PhysicalTypeCategory.CLASS);

    // Generates @RooJpaRepositoryCustomImpl annotation with implemented interface
    final AnnotationMetadataBuilder repositoryCustomImplAnnotationMetadata =
        new AnnotationMetadataBuilder(ROO_REPOSITORY_JPA_CUSTOM_IMPL);
    repositoryCustomImplAnnotationMetadata.addAttribute(new ClassAttributeValue(new JavaSymbolName(
        "repository"), interfaceType));
    ...
    getTypeManagementService().createOrUpdateTypeOnDisk(implBuilder.build());
