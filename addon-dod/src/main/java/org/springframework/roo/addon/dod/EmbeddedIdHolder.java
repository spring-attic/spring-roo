...
public class EmbeddedIdHolder {
	...
	public EmbeddedIdHolder(final FieldMetadata embeddedIdField, final List<FieldMetadata> idFields) {
		...
		Assert.notNull(idFields, "Fields for " + embeddedIdField.getFieldType().getFullyQualifiedTypeName() + " required");
	...
	public JavaSymbolName getEmbeddedIdMutator() {
		return new JavaSymbolName("set" + StringUtils.capitalize(embeddedIdField.getFieldName().getSymbolName()));
	}
