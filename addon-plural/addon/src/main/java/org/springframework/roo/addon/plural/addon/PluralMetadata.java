...
public class PluralMetadata extends AbstractItdTypeDetailsProvidingMetadataItem {
  ...
  public String getPlural(final FieldMetadata field) {
    ...
    final AnnotationMetadata annotation =
        MemberFindingUtils.getAnnotationOfType(field.getAnnotations(), ROO_PLURAL);
    if (annotation != null) {
      // Use the plural the user defined via the annotation
      final AnnotationAttributeValue<?> attribute =
          annotation.getAttribute(new JavaSymbolName("value"));
      if (attribute != null) {
  ...
  public String toString() {
    ...
    builder.append("cachedLookups", cache == null ? "[None]" : cache.keySet().toString());
