...
public class IntegrationTestOperationsImpl implements IntegrationTestOperations {
  ...
  public void newMockTest(final JavaType entity) {
    Validate.notNull(entity, "Entity to produce a mock test for is required");

    final JavaType name = new JavaType(entity + "Test");
    final String declaredByMetadataId =
        PhysicalTypeIdentifier.createIdentifier(name,
            Path.SRC_TEST_JAVA.getModulePathId(projectOperations.getFocusedModuleName()));
    ...
    if (cid != null) {
      ...
      if (countMethods.size() == 1) {
        final String countMethod =
            entity.getSimpleTypeName() + "." + countMethods.get(0).getMethodName().getSymbolName()
                + "()";
  ...
  public void newTestStub(final JavaType javaType) {
    Validate.notNull(javaType, "Class to produce a test stub for is required");
    ...
    final MemberDetails memberDetails =
        memberDetailsScanner.getMemberDetails(this.getClass().getName(), governorTypeDetails);
    for (final MemberHoldingTypeDetails typeDetails : memberDetails.getDetails()) {
      if (!(typeDetails.getCustomData().keySet().contains(CustomDataKeys.PERSISTENT_TYPE) || typeDetails
          .getDeclaredByMetadataId().startsWith(
              "MID:org.springframework.roo.addon.javabean.addon.ToStringMetadata"))) {
        for (final MethodMetadata method : typeDetails.getDeclaredMethods()) {
          ...
          if (Modifier.isPublic(method.getModifier()) && !Modifier.isAbstract(method.getModifier())) {
            ...
            final MethodMetadataBuilder methodBuilder =
                new MethodMetadataBuilder(declaredByMetadataId, Modifier.PUBLIC,
                    method.getMethodName(), JavaType.VOID_PRIMITIVE, bodyBuilder);
            methodBuilder.setAnnotations(methodAnnotations);
            methods.add(methodBuilder);
          }
        }
    ...
    if (!methods.isEmpty()) {
      ...
      final FieldMetadataBuilder fieldBuilder = new FieldMetadataBuilder(declaredByMetadataId);
      fieldBuilder.setModifier(Modifier.PRIVATE);
      fieldBuilder.setFieldName(new JavaSymbolName(StringUtils.uncapitalize(javaType
          .getSimpleTypeName())));
