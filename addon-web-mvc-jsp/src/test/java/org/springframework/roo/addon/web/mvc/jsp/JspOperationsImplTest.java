...
public class JspOperationsImplTest {
	 ...
	 */
	private void assertFolderAndMapping(final String preferredMapping, final String expectedFolder, final String expectedMapping) {
		// Set up
		final JavaType mockController = mock(JavaType.class);
		when(mockController.getSimpleTypeName()).thenReturn("FooController");
		
		// Invoke
		final Pair<String, String> pair = JspOperationsImpl.getFolderAndMapping(preferredMapping, mockController);
