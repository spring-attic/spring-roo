...
public class ProxyConfigurationCommands implements CommandMarker {
	...
	public String proxyConfiguration() throws MalformedURLException {
		...
		if (p == null) {
			sb.append("                     *** Your system has no proxy setup ***").append(StringUtils.LINE_SEPARATOR);
			sb.append("http://download.oracle.com/javase/6/docs/technotes/guides/net/proxies.html offers useful information.").append(StringUtils.LINE_SEPARATOR);
			sb.append("For most people, simply edit /etc/java-6-openjdk/net.properties (or equivalent) and set the").append(StringUtils.LINE_SEPARATOR);
