...
@Service
public class GwtRequestMetadataNotificationListener implements MetadataNotificationListener {
	...
	public void notify(final String upstreamMID, final String downstreamMID) {
		...
		Assert.isTrue(PhysicalTypeIdentifierNamingUtils.isValid(GwtRequestMetadata.class.getName(), downstreamInstanceId));
		metadataService.evictAndGet(downstreamInstanceId);
	}
	
	private String getDownstreamInstanceId(final String upstreamDependency) {
		final ClassOrInterfaceTypeDetails upstreamType = typeLocationService.getTypeDetails(upstreamDependency);
		...
		if (upstreamType.getAnnotation(ROO_GWT_REQUEST) == null) {
			final String downstreamOfGwtEntity = getDownstreamOfGwtEntity(upstreamType.getType());
	...
	private String getDownstreamOfGwtEntityLayerComponent(final MemberHoldingTypeDetails upstreamType) {
		...
		if (!layerEntities.isEmpty()) {
			...
			for (final ClassOrInterfaceTypeDetails request : typeLocationService.findClassesOrInterfaceDetailsWithAnnotation(ROO_GWT_REQUEST)) {
				...
				if (entity != null && layerEntities.contains(entity.getType())) {
					// This layer component has an associated GWT request; make that request the downstream
	...
	private String getDownstreamOfGwtEntity(final JavaType upstreamType) {
		for (final ClassOrInterfaceTypeDetails request : typeLocationService.findClassesOrInterfaceDetailsWithAnnotation(ROO_GWT_REQUEST)) {
			final AnnotationMetadata gwtRequestAnnotation = request.getAnnotation(ROO_GWT_REQUEST);
			if (gwtRequestAnnotation != null) {
				final AnnotationAttributeValue<?> attributeValue = gwtRequestAnnotation.getAttribute("value");
				Assert.state(attributeValue != null, "The x annotation should have a '" + "value" + "' attribute");
				final String entityClass = GwtUtils.getStringValue(attributeValue);
				if (upstreamType.getFullyQualifiedTypeName().equals(entityClass)) {
					// The upstream type is an entity with an associated GWT request; make that request the downstream
					return getLocalMid(request.getDeclaredByMetadataId());
				}
			}
		}
	...
	private String getLocalMid(final String physicalTypeId) {
		final JavaType typeName = PhysicalTypeIdentifier.getJavaType(physicalTypeId);
		final LogicalPath typePath = PhysicalTypeIdentifier.getPath(physicalTypeId);
