(function(jQuery) {
	jQuery(document).ready(function() {
		...
		jQuery.validator.methods.number = function(value, element) {
			return this.optional(element) || Inputmask.isValid(value, { alias: "numeric"});
		};
      ...
      var localizedValue = Inputmask.unmask(value,
              { alias: "numeric", unmaskAsNumber: true});
      ...
      var localizedValue = Inputmask.unmask(value,
              { alias: "numeric", unmaskAsNumber: true});
			return this.optional(element) ||( localizedValue >= params[0] && localizedValue <= params[1] );
		};
		...
		jQuery("form.validate").each(function( index ) {
			...
			$form.validate({
				...
				unhighlight: function(element) {
					...
					$element.parent().find(
							'span.form-control-feedback.glyphicon-remove').remove();
					// limpiar errores
					if($element.parent('.input-group').length) {
						$element.parent('.input-group').parent().find(
							'span.help-block[id='+$element.attr('id')+'-error]').remove();
				...
				errorPlacement: function(error, element) {
					...
					if($element.parent('.input-group').length) {
						var $previousErrors = $element.parent('.input-group').parent().find(
							'span.help-block[id='+$element.attr('id')+'-error]');
					}else{
						var $previousErrors = $element.parent().find(
							'span.help-block[id='+$element.attr('id')+'-error]');
					}
