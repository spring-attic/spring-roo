          ...
          <li class="dropdown"> <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false" data-th-utext="${r"#{label_menu_entry(#{language_label})}"}"><span class="caret">&nbsp</span></a>
    			<ul class="dropdown-menu" id="languageFlags">
      				<#list languages as language>
        				<li id="${language.localePrefix}Flag"><a href="?lang=${language.localePrefix}"><img class="flag" src="public/img/${language.localePrefix}.png" alt="${language.language}" />&nbsp;<span data-th-text="${r"#{language_label_"}${language.localePrefix}${r"}"}">${language.language}</span></a>
        				</li>
      				</#list>
    			</ul>
		  </li>
