...
/**
 * Configuration object for an action that can be executed
 * by the a {@link CommandConfiguration}.
 * 
 * Provides some default attributes with getters and setters that
 * are used by the default actions provided by tailor-core. 
 * Additional custom attributes for new actions can be added
 * via {@link #setAttribute(String, String)}. 
 * 
...
public class ActionConfig {
	...
	/**
	 * A map to flexibly define additional attributes for
	 * actions not delivered with tailor-core.
	 */
	 ...
	 * 
	 * @param actionTypeId
	 *            ID of the action's type This should be
	...
	/**
	 * @param module
	 *            Name of a module (e.g. used for FocusModule)
	 ...
	 * This method will throw an exception if a value with that key is
	 * already set, thus making them immutable. This is to avoid
	 * unexpected behaviour, because ActionConfigs in a TailorConfiguration
	 * must stay the same once that configuration is initiated at Roo startup.
	 * 
	 * @param argument
	 *            Argument name (e.g. used for DefaultValue action to define
	 *            a default value for an argument with this name)
	...
	/**
	 * @param value
	 *            Sets an attribute called value, e.g. used for 
	 *            DefaultValue action as default value
	 * 
	 * @see org.springframework.roo.addon.tailor.actions.DefaultValue
	 */
	...
	public boolean isMandatory() {
		String isMandatory = attributes.get(ATTR_MANDATORY);
		return "true".equals(isMandatory) || "yes".equals(isMandatory);
	}

	/**
	 * @param isMandatory
	 *            Sets an attribute called mandatory, e.g. used for DefaultValue
	 *            action to determine if a default value is mandatory or only
	 *            applies when not specified. Default is false.
	 * 
	 * @see org.springframework.roo.addon.tailor.actions.DefaultValue
	 */
	...
	/**
	 * @param command
	 *            An additional command to be executed. String can contain
	...
	/**
	 * @return A map to flexibly define additional attributes for
	 * actions not delivered with tailor-core.
	 */
	...
	public String toString() {
		...
		while(iterator.hasNext()) {
			...
			result.append(" | ").append(attribute).append(" = ").append(attributes.get(attribute));
		}
