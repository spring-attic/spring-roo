...
public class DefaultConfigurationLocator implements ConfigurationLocator {
	...
	/**
	 * Map of all available configurations - dynamically bound by Felix on startup
	 */
	...
	protected void bindConfig(TailorConfigurationFactory factory) {
		...
		if (config != null) {
			if (configurations.get(config.getName()) != null) {
				LOGGER.warning("TailorConfiguration duplicate '" + config.getName() + "', not binding again: " + config.toString());
			}
	...
	protected void unbindConfig(TailorConfigurationFactory factory) {
		// TODO: It's a little unelegant to call "create" method here again, but we need the name...
		TailorConfiguration config = factory.createTailorConfiguration();
		if (config != null) {
