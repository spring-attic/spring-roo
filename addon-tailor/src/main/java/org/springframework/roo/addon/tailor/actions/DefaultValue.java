...
/**
 * Adds default argument to the command
 * If default argument is mandatory it will be always replaced 
 * 
...
public class DefaultValue extends AbstractAction {
	...
	public void executeImpl(CommandTransformation arg, ActionConfig config) {
		...
		if (!arg.getInputCommand().contains("--" + argumentName)) {
			arg.setInputCommand(arg.getInputCommand()
					.concat(" --")
					.concat(argumentName)
					.concat(" ")
					.concat(config.getDefaultValue()));
			// Update the arguments, so that subsequent actions will be based on this default value
	...
	public boolean isValid(ActionConfig config) {
		return config != null 
			&& StringUtils.hasText(config.getArgument())
