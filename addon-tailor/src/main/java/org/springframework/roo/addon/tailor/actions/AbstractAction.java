...
public abstract class AbstractAction implements Action {
	...
	private static final Logger LOGGER = HandlerUtils.getLogger(AbstractAction.class);
	...
	public void execute(CommandTransformation command, ActionConfig config) {
		if (isValid(config)) {
			ActionConfig processedConfig = processConfigAttributes(command, config);
	...
	private ActionConfig processConfigAttributes(CommandTransformation command, ActionConfig config) {
		// Process variables in config
		ActionConfig processedConfig = new ActionConfig(config.getActionTypeId());
	 ...
	 */
	protected abstract void executeImpl(CommandTransformation command, ActionConfig config);
