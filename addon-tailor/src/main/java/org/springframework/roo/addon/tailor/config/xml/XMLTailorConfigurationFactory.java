...
/**
 * Factory to create a TailorConfiguration from an XML configuration file
 * named "tailor.xml" in the shell root.
 *
...
public class XMLTailorConfigurationFactory implements
	...
	protected void activate(final ComponentContext context) {
		...
		// Use this instead of the shell object, because shell.getHome() sometimes
		// gives false results, e.g. when running Roo in test mode from Eclipse
		// (PathResolver cannot be used because the config file needs to be available even
		// if there is no project created)
		final File shellDirectory = new File(StringUtils.defaultIfEmpty(OSGiUtils.getRooWorkingDirectory(context), CURRENT_DIRECTORY));
	...
	public TailorConfiguration createTailorConfiguration() {
		...
		try {
			Document readXml = XmlUtils.readXml(fileManager.getInputStream(configFileIdentifier));
			Element root = readXml.getDocumentElement();
			return mapXmlToTailorConfiguration(root);
		} catch(Exception e) {
			// Make sure that an invalid tailor.xml file does not crash the whole shell
	...
	private void logTailorXMLInvalid(String msg) {
		LOGGER.warning("Invalid tailor.xml - please correct and restart the shell to use this configuration (" + msg + ")");
	}
	...
	private TailorConfiguration mapXmlToTailorConfiguration(Element root) {
		...
		List<Element> elTailors = XmlUtils.findElements("/tailorconfiguration/tailor", root);
		// TODO: Currently only one tailor supported in the configuration file
		// > Should be extended to support multiple tailors, the XML already supports this, but is not evaluated
		...
		TailorConfiguration result = new TailorConfiguration(elTailor.getAttribute("name"),
				elTailor.getAttribute("description"));
		
		List<Element> elConfigs = XmlUtils.findElements("/tailorconfiguration/tailor/config", root);
		...
		for (Element elConfig : elConfigs) {
			...
			for (Element elAction : elActions) {
				...
				ActionConfig newAction = new ActionConfig(elAction.getAttribute("type"));
